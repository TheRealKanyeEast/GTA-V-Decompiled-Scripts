#region Local Var
	var uLocal_0 = 0;
	var uLocal_1 = 0;
	int iLocal_2 = 0;
	int iLocal_3 = 0;
	int iLocal_4 = 0;
	int iLocal_5 = 0;
	int iLocal_6 = 0;
	int iLocal_7 = 0;
	int iLocal_8 = 0;
	int iLocal_9 = 0;
	int iLocal_10 = 0;
	int iLocal_11 = 0;
	float fLocal_12 = 0f;
	var uLocal_13 = 0;
	var uLocal_14 = 0;
	int iLocal_15 = 0;
	int iLocal_16 = 0;
	var uLocal_17 = 0;
	var uLocal_18 = 0;
	char* sLocal_19 = 0;
	var uLocal_20 = 0;
	var uLocal_21 = 0;
	float fLocal_22 = 0f;
	float fLocal_23 = 0f;
	float fLocal_24 = 0f;
	var uLocal_25 = 0;
	var uLocal_26 = 0;
	float fLocal_27 = 0f;
	var uLocal_28 = 0;
	var uLocal_29 = 0;
	var uLocal_30 = 0;
	float fLocal_31 = 0f;
	float fLocal_32 = 0f;
	var uLocal_33 = 0;
	var uLocal_34 = 0;
	int iLocal_35 = 0;
	var uLocal_36 = 0;
	var uLocal_37 = 0;
	var uLocal_38 = 0;
	int iLocal_39 = 0;
	int iLocal_40 = 0;
	int iLocal_41 = 0;
	int iLocal_42 = 0;
	var uLocal_43 = 0;
	var uLocal_44 = 0;
	var uLocal_45 = 0;
	var uLocal_46 = 0;
	var uLocal_47 = 0;
	var uLocal_48 = 0;
	var uLocal_49 = 0;
	var uLocal_50 = 0;
	var uLocal_51 = 0;
	var uLocal_52 = 0;
	var uLocal_53 = 0;
	var uLocal_54 = 0;
	var uLocal_55 = 0;
	var uLocal_56 = 0;
	var uLocal_57 = 0;
	var uLocal_58 = 0;
	var uLocal_59 = 0;
	var uLocal_60 = 0;
	var uLocal_61 = 0;
	float fLocal_62 = 0f;
	var uLocal_63 = 0;
	float fLocal_64 = 0f;
	float fLocal_65 = 0f;
	float fLocal_66 = 0f;
	float fLocal_67 = 0f;
	float fLocal_68 = 0f;
	float fLocal_69 = 0f;
	int iLocal_70 = 0;
	int iLocal_71 = 0;
	var uLocal_72 = 0;
	BOOL bLocal_73 = 0;
	int iLocal_74 = 0;
	BOOL bLocal_75 = 0;
	var uLocal_76 = 0;
	var uLocal_77 = 0;
	var uLocal_78 = 0;
	int iLocal_79 = 0;
	int iLocal_80 = 0;
	int iLocal_81 = 0;
	float fLocal_82 = 0f;
	var uLocal_83 = 0;
	var uLocal_84 = 0;
	int iLocal_85 = 0;
	BOOL bLocal_86 = 0;
	int iLocal_87 = 0;
	int iLocal_88 = 0;
	int iLocal_89 = 0;
	int iLocal_90 = 0;
	int iLocal_91 = 0;
	int iLocal_92 = 0;
	var uLocal_93 = 0;
	var uLocal_94 = 0;
	var uLocal_95 = 0;
	var uLocal_96 = 0;
	BOOL bLocal_97 = 0;
	int iLocal_98 = 0;
	var uLocal_99 = 0;
	var uLocal_100 = 0;
	int iLocal_101 = 0;
	var uLocal_102 = 0;
	int iLocal_103 = 0;
	int iLocal_104 = 0;
	var uLocal_105 = 0;
	int iLocal_106 = 0;
	int iLocal_107 = 0;
	int iLocal_108 = 0;
	int iLocal_109 = 0;
	var uLocal_110 = 0;
	BOOL bLocal_111 = 0;
	int iLocal_112 = 0;
	var uLocal_113 = 0;
	int iLocal_114 = 0;
	int iLocal_115 = 0;
	BOOL bLocal_116 = 0;
	int iLocal_117 = 0;
	var uLocal_118 = 0;
	var uLocal_119 = 0;
	var uLocal_120 = 0;
	int iLocal_121 = 0;
	BOOL bLocal_122 = 0;
	int iLocal_123 = 0;
#endregion

void main() // Position - 0x0
{
	var unk;
	var unk17;
	int num;
	float num2;
	float num3;
	int num4;
	int num5;
	BOOL flag;
	int num6;

	iLocal_2 = 1;
	iLocal_3 = 134;
	iLocal_4 = 134;
	iLocal_5 = 1;
	iLocal_6 = 1;
	iLocal_7 = 1;
	iLocal_8 = 134;
	iLocal_9 = 1;
	iLocal_10 = 12;
	iLocal_11 = 12;
	fLocal_12 = 0.001f;
	iLocal_15 = -1;
	sLocal_19 = "NULL";
	fLocal_22 = 80f;
	fLocal_23 = 140f;
	fLocal_24 = 180f;
	fLocal_27 = 0f;
	fLocal_31 = -0.0375f;
	fLocal_32 = 0.17f;
	iLocal_35 = 3;
	iLocal_39 = 1;
	iLocal_40 = 65;
	iLocal_41 = 49;
	iLocal_42 = 64;
	fLocal_62 = (0.05f + 0.275f) - 0.01f;
	fLocal_64 = 0.73f;
	fLocal_65 = 0.55f;
	fLocal_66 = 0.73f;
	fLocal_67 = 0.45f;
	fLocal_68 = 0f;
	fLocal_69 = 0f;
	unk_0x51CC1333A10C4E09();
	Global_20701 = 145;
	unk_0x8744D2E3FC95740E(&Global_8371, 8);
	unk_0x8744D2E3FC95740E(&Global_8370, 14);
	unk_0x8744D2E3FC95740E(&Global_4542302, 3);
	unk_0x8744D2E3FC95740E(&Global_8371, 10);
	unk_0x8744D2E3FC95740E(&Global_8370, 17);
	unk_0x8744D2E3FC95740E(&Global_8370, 9);
	unk_0x8744D2E3FC95740E(&Global_8370, 26);
	unk_0x8744D2E3FC95740E(&Global_8370, 23);
	unk_0x8744D2E3FC95740E(&Global_8371, 24);
	unk_0x8744D2E3FC95740E(&Global_8371, 25);
	unk_0x8744D2E3FC95740E(&Global_8371, 27);
	unk_0x8744D2E3FC95740E(&Global_8371, 26);
	unk_0x8744D2E3FC95740E(&Global_8370, 30);
	Global_2804775 = 0;
	iLocal_121 = 0;
	unk_0x4FA503A6757253A7(0, 0);
	unk_0xFC75D88023832ED9(0);
	Global_20466 = 0;
	bLocal_73 = false;
	Global_20444 = 0;
	Global_20484 = 0;
	Global_20485 = 0;
	func_123();
	Global_20431 = { Global_20463 };
	Global_20504 = 4;
	Global_20505 = 0;
	Global_8975 = 1;
	Global_20480 = Global_20504;

	if (Global_20482 == false)
	{
		Global_20483 = 0;
	
		if (Global_78689)
		{
			if (func_122())
				Global_20481 = unk_0x8DE4F68A9728925E("cellphone_badger");
			else
				Global_20481 = unk_0x8DE4F68A9728925E("cellphone_ifruit");
		
			iLocal_87 = 1;
			iLocal_87 == 1;
		}
		else if (Global_20443)
		{
			Global_20488 = true;
			Global_20481 = unk_0x8DE4F68A9728925E("cellphone_prologue");
		}
		else
		{
			Global_20481 = unk_0x8DE4F68A9728925E(&Global_113810.f_14054[Global_20500 /*20*/]);
		}
	
		SYSTEM::SETTIMERA(0);
	
		while (!unk_0xA0C7B98BCF1EEF9E(Global_20481) && Global_20483 == 0)
		{
			SYSTEM::WAIT(0);
		
			if (SYSTEM::TIMERA() > 20000)
				Global_20483 = 1;
		
			if (unk_0x761778199FE1211C())
				if (unk_0xD130E7CDEE903624(unk_0x4A8C381C258A124D(), "Synched"))
					if (SYSTEM::TIMERA() > 2000)
						Global_20483 = 1;
		
			if (Global_20499 == 1)
				func_121();
		}
	
		if (Global_20483 == 1)
		{
			if (unk_0xA0C7B98BCF1EEF9E(Global_20481))
				func_120(Global_20481, "SHUTDOWN_MOVIE");
		
			SYSTEM::WAIT(0);
			unk_0xD314260005F064BF(&Global_20481);
			Global_20441 = 0;
			Global_20482 = false;
			Global_20702 = 0;
		
			if (Global_113810.f_14054.f_84 == 1)
			{
				Global_113810.f_14054.f_84 = 0;
				unk_0x665A7E873A6664BC(0, 1);
			}
		
			Global_20444 = 1;
			Global_20500.f_1 = 3;
			func_119();
			Global_20488 = false;
			unk_0x5F2E2CEDE90F627E();
			unk_0x8744D2E3FC95740E(&Global_8370, 9);
			unk_0x8744D2E3FC95740E(&Global_8370, 27);
			unk_0x8744D2E3FC95740E(&Global_8370, 30);
			unk_0x8744D2E3FC95740E(&Global_8371, 5);
			unk_0x8744D2E3FC95740E(&Global_8371, 21);
			unk_0x8744D2E3FC95740E(&Global_8372, 2);
		
			if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
				unk_0x92187CF73A8D1B96(unk_0x4A8C381C258A124D());
		
			if (unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
				unk_0x92187CF73A8D1B96(unk_0x4A8C381C258A124D());
		
			Global_20704 = false;
			Global_22905 = 0;
			Global_22904 = 0;
			Global_21858 = false;
			func_117();
			func_115();
			Global_4542581 = 0;
			unk_0xBBC29EBE6E1A48FA();
		}
	
		Global_20424 = 0.1f;
		Global_20425 = 0.38f;
		Global_20426 = 0.195f;
		Global_20427 = 0.05f;
	
		if (unk_0x1226C55CA7D2269A() || func_114() || unk_0x761778199FE1211C())
		{
			unk_0xA306E6FD2A6558E6(18, &iLocal_107, &iLocal_108, &iLocal_109, &uLocal_110);
			func_113(Global_20481, "SET_SOFT_KEYS_COLOUR", 2f, SYSTEM::TO_FLOAT(iLocal_107), SYSTEM::TO_FLOAT(iLocal_108), SYSTEM::TO_FLOAT(iLocal_109), -1082130432);
			unk_0xA306E6FD2A6558E6(9, &iLocal_107, &iLocal_108, &iLocal_109, &uLocal_110);
			func_113(Global_20481, "SET_SOFT_KEYS_COLOUR", 1f, SYSTEM::TO_FLOAT(iLocal_107), SYSTEM::TO_FLOAT(iLocal_108), SYSTEM::TO_FLOAT(iLocal_109), -1082130432);
			unk_0xA306E6FD2A6558E6(6, &iLocal_107, &iLocal_108, &iLocal_109, &uLocal_110);
			func_113(Global_20481, "SET_SOFT_KEYS_COLOUR", 3f, SYSTEM::TO_FLOAT(iLocal_107), SYSTEM::TO_FLOAT(iLocal_108), SYSTEM::TO_FLOAT(iLocal_109), -1082130432);
		}
		else
		{
			if (!unk_0x74C1828126652223())
			{
				unk_0xA306E6FD2A6558E6(9, &iLocal_107, &iLocal_108, &iLocal_109, &uLocal_110);
				func_113(Global_20481, "SET_SOFT_KEYS_COLOUR", 2f, SYSTEM::TO_FLOAT(iLocal_107), SYSTEM::TO_FLOAT(iLocal_108), SYSTEM::TO_FLOAT(iLocal_109), -1082130432);
				unk_0xA306E6FD2A6558E6(126, &iLocal_107, &iLocal_108, &iLocal_109, &uLocal_110);
				func_113(Global_20481, "SET_SOFT_KEYS_COLOUR", 1f, SYSTEM::TO_FLOAT(iLocal_107), SYSTEM::TO_FLOAT(iLocal_108), SYSTEM::TO_FLOAT(iLocal_109), -1082130432);
				unk_0xA306E6FD2A6558E6(6, &iLocal_107, &iLocal_108, &iLocal_109, &uLocal_110);
				func_113(Global_20481, "SET_SOFT_KEYS_COLOUR", 3f, SYSTEM::TO_FLOAT(iLocal_107), SYSTEM::TO_FLOAT(iLocal_108), SYSTEM::TO_FLOAT(iLocal_109), -1082130432);
			}
		
			if (unk_0x74C1828126652223())
			{
				unk_0xA306E6FD2A6558E6(6, &iLocal_107, &iLocal_108, &iLocal_109, &uLocal_110);
				func_113(Global_20481, "SET_SOFT_KEYS_COLOUR", 2f, SYSTEM::TO_FLOAT(iLocal_107), SYSTEM::TO_FLOAT(iLocal_108), SYSTEM::TO_FLOAT(iLocal_109), -1082130432);
				unk_0xA306E6FD2A6558E6(126, &iLocal_107, &iLocal_108, &iLocal_109, &uLocal_110);
				func_113(Global_20481, "SET_SOFT_KEYS_COLOUR", 1f, SYSTEM::TO_FLOAT(iLocal_107), SYSTEM::TO_FLOAT(iLocal_108), SYSTEM::TO_FLOAT(iLocal_109), -1082130432);
				unk_0xA306E6FD2A6558E6(9, &iLocal_107, &iLocal_108, &iLocal_109, &uLocal_110);
				func_113(Global_20481, "SET_SOFT_KEYS_COLOUR", 3f, SYSTEM::TO_FLOAT(iLocal_107), SYSTEM::TO_FLOAT(iLocal_108), SYSTEM::TO_FLOAT(iLocal_109), -1082130432);
			}
		}
	
		Global_20483 == 0;
		func_112();
	
		if (Global_78689)
			TEXT_LABEL_ASSIGN_STRING(&Global_20489, "Phone_SoundSet_Michael", 24);
	
		Global_20482 = true;
	}

	Global_8373 = 99;
	func_111();

	if (unk_0x75EAB09F5E974116(unk_0x259BE71D8A81D4FA()))
		uLocal_99 = unk_0x8D91ADE44AC79BC9(unk_0x4A8C381C258A124D());

	if (unk_0xBE259DBA45F2996E())
		Global_22859 = 1;
	else
		Global_22859 = 0;

	func_113(Global_20481, "SET_SLEEP_MODE", 0f, -1082130432, -1082130432, -1082130432, -1082130432);
	Global_20442 = 1;

	if (Global_78689)
	{
		Global_20706 = false;
	
		if (func_122())
			if (Global_1836503 == 1)
				Global_4542578 = 4;
			else
				Global_4542578 = 2;
		else
			Global_4542578 = func_107(2030, -1);
	
		if (Global_4542578 < 1 || Global_4542578 > 7)
			Global_4542578 = 1;
	
		func_113(Global_20481, "SET_THEME", SYSTEM::TO_FLOAT(Global_4542578), -1082130432, -1082130432, -1082130432, -1082130432);
		func_106();
	
		if (func_122())
			if (Global_1836503 == 1)
				Global_4542579 = 16;
			else
				Global_4542579 = 9;
		else
			Global_4542579 = func_107(2029, -1);
	
		if (Global_4542579 == 0)
		{
			unk17 = { func_105(unk_0x259BE71D8A81D4FA()) };
			num = 0;
			unk_0x97DD063A9C6137F8(0) == 0;
			Global_4542582 == 1;
		
			if (unk_0xE582BF3EDDBB1A68(&unk17) && unk_0x97DD063A9C6137F8(0) && Global_4542582 == 0)
			{
				if (unk_0xE460920F3D75C34D(&unk17, &unk))
				{
				}
				else
				{
					num = 1;
				}
			
				if (num == 0)
				{
					unk_0x88F483FBD433696A(Global_20481, "SET_BACKGROUND_CREW_IMAGE");
					unk_0x882AEFD55B8D51FB("CELL_2000" /*~a~*/);
					unk_0x60D332F23943B34F(&unk);
					unk_0xCFAD3D478C87321A();
					unk_0xE6B753D52F4CA222();
				}
				else
				{
					func_113(Global_20481, "SET_BACKGROUND_IMAGE", SYSTEM::TO_FLOAT(0), -1082130432, -1082130432, -1082130432, -1082130432);
				}
			}
			else
			{
				func_113(Global_20481, "SET_BACKGROUND_IMAGE", SYSTEM::TO_FLOAT(0), -1082130432, -1082130432, -1082130432, -1082130432);
			}
		}
		else
		{
			func_113(Global_20481, "SET_BACKGROUND_IMAGE", SYSTEM::TO_FLOAT(Global_4542579), -1082130432, -1082130432, -1082130432, -1082130432);
		}
	}
	else
	{
		if (Global_20443)
			Global_20706 = false;
		else if (Global_113810.f_14054.f_88 == 1 || Global_113810.f_14054.f_89 == true)
			Global_20706 = false;
		else
			Global_20706 = false;
	
		func_113(Global_20481, "SET_THEME", SYSTEM::TO_FLOAT(Global_113810.f_14054[Global_20500 /*20*/].f_6), -1082130432, -1082130432, -1082130432, -1082130432);
	
		if (Global_9174 == 0)
			func_113(Global_20481, "SET_BACKGROUND_IMAGE", SYSTEM::TO_FLOAT(Global_113810.f_14054[Global_20500 /*20*/].f_9), -1082130432, -1082130432, -1082130432, -1082130432);
		else
			unk_0xBA290C9390813C9F(575f);
	
		func_106();
	}

	iLocal_88 = func_104();

	if (Global_20443 == false)
		func_92();

	Global_20478 = 0;
	Global_20705 = false;
	unk_0x8744D2E3FC95740E(&Global_8370, 9);
	Global_2694571 = false;

	if (func_87(0) && unk_0x261E3728EE56B3AC())
		Global_2694571 = true;

	func_113(Global_20481, "SET_DATA_SLOT_EMPTY", 1f, -1082130432, -1082130432, -1082130432, -1082130432);
	func_85();
	func_84();
	func_83(1);
	unk_0xBF3D28CA44F3BE2D(-1, "Pull_Out", &Global_20489, 1);
	SYSTEM::SETTIMERB(0);

	while (Global_20500.f_1 < 6 && Global_20483 == 0)
	{
		SYSTEM::WAIT(0);
	
		if (SYSTEM::TIMERB() > 10000)
			Global_20483 = 1;
	
		if (Global_20500.f_1 < 4)
			Global_20483 = 1;
	}

	if (Global_20500.f_1 == 5 || Global_20500.f_1 == 6)
	{
		if (func_82(14))
			func_113(Global_20481, "DISPLAY_VIEW", 1f, SYSTEM::TO_FLOAT(6), -1082130432, -1082130432, -1082130432);
		else
			func_113(Global_20481, "DISPLAY_VIEW", 1f, SYSTEM::TO_FLOAT(Global_20504), -1082130432, -1082130432, -1082130432);
	
		if (Global_9175[Global_20500 /*2811*/][0 /*281*/].f_259 == 2)
			func_113(Global_20481, "SET_SLEEP_MODE", 1f, -1082130432, -1082130432, -1082130432, -1082130432);
		else
			func_113(Global_20481, "SET_SLEEP_MODE", 0f, -1082130432, -1082130432, -1082130432, -1082130432);
	
		if (Global_20488)
			func_81(Global_20481, "SET_SOFT_KEYS", 2f, 1f, 2f, -1f, -1f, "CELL_205" /*SELECT*/, 0, 0, 0, 0);
		else
			func_81(Global_20481, "SET_SOFT_KEYS", 2f, 1f, 2f, -1f, -1f, 0, 0, 0, 0, 0);
	
		if (Global_20488)
			func_81(Global_20481, "SET_SOFT_KEYS", 3f, 1f, 4f, -1f, -1f, "CELL_206" /*BACK*/, 0, 0, 0, 0);
		else
			func_81(Global_20481, "SET_SOFT_KEYS", 3f, 1f, 4f, -1f, -1f, 0, 0, 0, 0, 0);
	
		if (Global_20706 == false)
		{
			func_81(Global_20481, "SET_SOFT_KEYS", 1f, 0f, 3f, -1f, -1f, 0, 0, 0, 0, 0);
			unk_0x8744D2E3FC95740E(&Global_8370, 17);
		}
		else if (Global_78689)
		{
			func_81(Global_20481, "SET_SOFT_KEYS", 1f, 0f, 3f, -1f, -1f, 0, 0, 0, 0, 0);
			unk_0x8744D2E3FC95740E(&Global_8370, 17);
		}
		else
		{
			if (Global_20705 == true)
				if (Global_20488)
					func_81(Global_20481, "SET_SOFT_KEYS", 1f, 1f, 20f, -1f, -1f, "CELL_225" /*NEXT*/, 0, 0, 0, 0);
				else
					func_81(Global_20481, "SET_SOFT_KEYS", 1f, 1f, 20f, -1f, -1f, 0, 0, 0, 0, 0);
			else if (Global_20488)
				func_81(Global_20481, "SET_SOFT_KEYS", 1f, 1f, 3f, -1f, -1f, "CELL_225" /*NEXT*/, 0, 0, 0, 0);
			else
				func_81(Global_20481, "SET_SOFT_KEYS", 1f, 1f, 3f, -1f, -1f, 0, 0, 0, 0, 0);
		
			unk_0x0B0C9A0F9AAEB7F0(&Global_8370, 17);
		}
	
		func_80();
	}

	Global_20486 = 1;
	func_79();

	if (unk_0x76CD105BCAC6EB9F())
		uLocal_105 = unk_0x7E3F74F641EE6B27();
	else
		uLocal_100 = unk_0x1DD05E817C89C737();

	if (Global_20702 == 1)
	{
		if (Global_9175[Global_20500 /*2811*/][0 /*281*/].f_259 == 2)
		{
			if (Global_113810.f_14054.f_84 == 0)
			{
				Global_113810.f_14054.f_84 = 1;
			
				if (Global_20445 == 0)
				{
					num2 = 0.75f;
					num3 = 0.8f;
				}
				else
				{
					num2 = 0.65f;
					num3 = 0.77f;
				}
			
				num2 == num3;
				func_78("CELL_7052" /*Sleep Mode is active.~n~The game story is paused. To continue it, enter the SETTINGS menu, select PROFILE then NORMAL MODE or trigger an existing mission.*/, 10000);
			}
		}
	}

	if (Global_78689 == false)
		iLocal_16 = unk_0x1DD05E817C89C737() + 375;

	if (func_77())
		iLocal_79 = 1;
	else
		iLocal_79 = 0;

	if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
	{
		if (unk_0xD6179D448A06A77F(unk_0x4A8C381C258A124D(), 0))
		{
			iLocal_80 = 1;
		
			if (func_77())
				func_76();
		}
		else
		{
			iLocal_80 = 0;
		}
	}

	if (unk_0x761778199FE1211C())
	{
		if (Global_78689 == false)
		{
			uLocal_102 = unk_0x1DD05E817C89C737();
			iLocal_104 = 0;
		}
	}

	unk_0x8744D2E3FC95740E(&Global_8372, 9);

	while (true)
	{
		SYSTEM::WAIT(0);
	
		if (IS_BIT_SET(Global_4542302, 24))
		{
			if (!Global_20705)
			{
				if (Global_20500.f_1 == 6 || Global_20500.f_1 > 6)
				{
					unk_0x8744D2E3FC95740E(&Global_4542302, 24);
					func_85();
					func_113(Global_20481, "SET_DATA_SLOT_EMPTY", 1f, -1082130432, -1082130432, -1082130432, -1082130432);
					func_83(1);
				
					if (Global_20500.f_1 == 6)
						func_113(Global_20481, "DISPLAY_VIEW", 1f, SYSTEM::TO_FLOAT(Global_20480), -1082130432, -1082130432, -1082130432);
				}
			}
		}
	
		if (func_75())
			unk_0x66EFB3D6110055C4(0, 114, 1);
	
		if (Global_78689 == false)
		{
			if (bLocal_75)
				if (Global_20500.f_1 == 6 || Global_20500.f_1 == 7)
					bLocal_75 = false;
		
			if (Global_20500.f_1 > 4)
			{
				if (IS_BIT_SET(Global_8370, 14) && Global_4542581 == 0 && Global_20440 == 0)
				{
					if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
					{
						if (func_74())
						{
							unk_0x78C4EBB0251847E2(&uLocal_76);
						
							if (Global_20446[Global_20445 /*3*/].f_1 != uLocal_76.f_1)
								func_72();
						}
						else
						{
							unk_0x78C4EBB0251847E2(&uLocal_76);
						
							if (Global_20453[Global_20445 /*3*/].f_1 != uLocal_76.f_1)
							{
								!func_75();
							
								if (unk_0x486FF5D06E9659F1(joaat("appemail")) < 1)
									func_70();
							}
						}
					
						if (iLocal_80 == 0)
						{
							if (unk_0xD6179D448A06A77F(unk_0x4A8C381C258A124D(), 0))
							{
								iLocal_80 = 1;
								func_70();
								func_76();
							}
						}
						else if (!unk_0xD6179D448A06A77F(unk_0x4A8C381C258A124D(), 0))
						{
							iLocal_80 = 0;
							func_72();
						
							if (Global_20500.f_1 == 10 || Global_20500.f_1 == 9)
								if (Global_21845 != 2)
									func_69();
						}
					
						if (iLocal_79 == 0)
						{
							if (func_77())
							{
								iLocal_79 = 1;
							
								if (unk_0xD6179D448A06A77F(unk_0x4A8C381C258A124D(), 0))
									func_76();
							
								func_72();
							}
						}
						else
						{
							if (unk_0xC2BF1F6F84E31EB2(unk_0x4A8C381C258A124D()))
								if (Global_20500.f_1 == 10 || Global_20500.f_1 == 9)
									unk_0x66EFB3D6110055C4(0, 26, 1);
						
							if (func_77() == false)
							{
								iLocal_79 = 0;
								func_70();
							}
						}
					}
				}
			}
		}
	
		if (Global_2694571 == true)
		{
			if (!func_87(0) || !unk_0x261E3728EE56B3AC())
			{
				if (unk_0x486FF5D06E9659F1(joaat("appcamera")) == 0)
				{
					if (Global_20500.f_1 > 3)
					{
						Global_20485 = 1;
						func_68();
						func_65(0);
						Global_2694571 = false;
					}
				}
			}
		}
	
		if (Global_20707 == 0)
		{
			if (Global_43377 != 15 || Global_8375 == 1 || func_64(false))
			{
				if (!Global_20705)
				{
					if (Global_20500.f_1 == 6)
					{
						Global_8376 = 42;
						func_113(Global_20481, "SET_DATA_SLOT_EMPTY", 1f, -1082130432, -1082130432, -1082130432, -1082130432);
						func_85();
						func_84();
						func_83(1);
						func_113(Global_20481, "DISPLAY_VIEW", 1f, SYSTEM::TO_FLOAT(Global_20480), -1082130432, -1082130432, -1082130432);
					}
				}
			
				Global_20707 = 1;
			}
		}
		else if (Global_43377 == 15 && func_64(false) == false && Global_8375 == 0)
		{
			if (!Global_20705)
			{
				if (Global_20500.f_1 == 6)
				{
					Global_8376 = 255;
					func_113(Global_20481, "SET_DATA_SLOT_EMPTY", 1f, -1082130432, -1082130432, -1082130432, -1082130432);
					func_85();
					func_84();
					func_83(1);
					func_113(Global_20481, "DISPLAY_VIEW", 1f, SYSTEM::TO_FLOAT(Global_20480), -1082130432, -1082130432, -1082130432);
				}
			}
		
			Global_20707 = 0;
		}
	
		if (Global_20708 == 0)
		{
			if (IS_BIT_SET(Global_8371, 3))
			{
				if (!Global_20705)
				{
					if (Global_20500.f_1 == 6)
					{
						func_113(Global_20481, "SET_DATA_SLOT_EMPTY", 1f, -1082130432, -1082130432, -1082130432, -1082130432);
						func_85();
						func_84();
						func_83(1);
						func_113(Global_20481, "DISPLAY_VIEW", 1f, SYSTEM::TO_FLOAT(Global_20480), -1082130432, -1082130432, -1082130432);
					}
				}
			
				Global_20708 = 1;
			}
		}
		else if (!IS_BIT_SET(Global_8371, 3))
		{
			if (!Global_20705)
			{
				if (Global_20500.f_1 == 6)
				{
					func_113(Global_20481, "SET_DATA_SLOT_EMPTY", 1f, -1082130432, -1082130432, -1082130432, -1082130432);
					func_85();
					func_84();
					func_83(1);
					func_113(Global_20481, "DISPLAY_VIEW", 1f, SYSTEM::TO_FLOAT(Global_20480), -1082130432, -1082130432, -1082130432);
				}
			}
		
			Global_20708 = 0;
		}
	
		if (!unk_0x4D9174D8796EA622())
		{
			if (Global_20428.f_1 != Global_20446[Global_20445 /*3*/].f_1 || func_77())
			{
				if (Global_20500.f_1 > 3)
				{
					if (unk_0x486FF5D06E9659F1(joaat("apptrackify")) == 0)
						if (Global_78689)
							if (Global_20500.f_1 == 9)
								if (Global_21898 == 1 || IS_BIT_SET(Global_8371, 23))
									unk_0x4CC1CF98851922CE(0, Global_20469);
							else
								unk_0x4CC1CF98851922CE(0, Global_20469);
						else
							unk_0x4CC1CF98851922CE(0, Global_20469);
				
					if (unk_0x486FF5D06E9659F1(joaat("appsecurohack")) == 0)
					{
						unk_0x4CC1CF98851922CE(0, Global_20470);
					
						if (IS_BIT_SET(Global_8370, 17))
							unk_0x4CC1CF98851922CE(0, Global_20471);
					}
				
					if (!func_63() && unk_0x486FF5D06E9659F1(joaat("appsecurohack")) == 0)
						unk_0x4CC1CF98851922CE(0, Global_20472);
				}
			}
		
			Global_20428.f_1 == Global_20446[Global_20445 /*3*/].f_1;
		}
	
		if (!unk_0xAE231F549813BBDF(2))
		{
			unk_0x66EFB3D6110055C4(0, 140, 1);
			unk_0x66EFB3D6110055C4(0, 141, 1);
		}
	
		if (!unk_0x4D9174D8796EA622())
		{
			if (!Global_75816)
			{
				if (!func_75())
				{
					unk_0x4CC1CF98851922CE(0, 180);
					unk_0x4CC1CF98851922CE(0, 181);
					unk_0xB211E45F1FE1ED2D(0, 180, 1);
					unk_0xB211E45F1FE1ED2D(0, 181, 1);
				}
			
				unk_0x4CC1CF98851922CE(0, Global_20474);
			
				if (!func_63())
					unk_0x4CC1CF98851922CE(0, Global_20475);
			
				if (Global_20500.f_1 > 4)
					unk_0x4CC1CF98851922CE(0, Global_20477);
			}
		}
	
		if (func_62(2, Global_20468, 0))
		{
			bLocal_97 = true;
			SYSTEM::SETTIMERA(0);
		}
	
		if (iLocal_74 == 1)
		{
			if (!unk_0x6D05C5731A838CB3(2, Global_20469))
			{
				Global_8374 = 1;
				iLocal_74 = 0;
			}
		
			if (!(Global_20500.f_1 > 3))
				iLocal_74 = 0;
		}
	
		if (bLocal_97)
		{
			if (unk_0x6D05C5731A838CB3(2, Global_20468))
			{
				if (SYSTEM::TIMERA() > 5000)
				{
					Global_20500.f_1 = 3;
					func_60();
				}
			}
			else
			{
				bLocal_97 = false;
			}
		}
	
		Global_8978 = unk_0x1DD05E817C89C737();
	
		if (Global_20487)
		{
			func_79();
			Global_20486 = 1;
			Global_20487 = false;
		}
	
		if (Global_20704 == false)
		{
			unk_0x4EB223432F8FA0A0(6);
			unk_0x4EB223432F8FA0A0(7);
			unk_0x4EB223432F8FA0A0(8);
			unk_0x4EB223432F8FA0A0(9);
			func_53();
			func_52();
			unk_0x61055542D9D877AE(iLocal_85);
		
			if (Global_20486 == 1)
			{
				unk_0xA4F67CEB594AE064(4);
			
				if (Global_20443)
					unk_0x6D16B99FEB0AFFF1(Global_20481, Global_20420, Global_20421, Global_20422, Global_20423, 255, 255, 255, 255, 0);
				else
					unk_0x6D16B99FEB0AFFF1(Global_20481, Global_20420, Global_20421, Global_20422, Global_20423, 255, 255, 255, 255, 0);
			
				func_111();
			}
		
			if (Global_20440 == 1)
			{
				if (Global_20500.f_1 > 3)
					func_51();
			}
			else if (Global_20500.f_1 > 3)
			{
				if (Global_4542581 == 1)
					func_50();
			
				if (IS_BIT_SET(Global_8371, 25))
				{
					if (Global_78689 == true)
						func_49();
				}
				else if (!IS_BIT_SET(Global_8371, 24))
				{
					if (IS_BIT_SET(Global_8371, 26))
					{
						if (func_62(2, Global_20467, 0))
						{
							unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 25);
							unk_0x8744D2E3FC95740E(&Global_8371, 26);
							unk_0x8744D2E3FC95740E(&Global_8372, 2);
						}
					}
				}
				else if (Global_78689 == true)
				{
					func_48();
				}
			}
		}
	
		if (bLocal_73)
		{
			if (Global_20500.f_1 == 6)
				func_44();
		}
		else if (!IS_BIT_SET(Global_8370, 23))
		{
			if (Global_20500.f_1 == 5 || Global_20500.f_1 == 6)
			{
				if (Global_20704 == false)
				{
					if (unk_0x486FF5D06E9659F1(joaat("appcamera")) < 1 && unk_0x486FF5D06E9659F1(joaat("appemail")) < 1)
					{
						if (Global_20443 == false)
							if (IS_BIT_SET(Global_8370, 14))
								if (!Global_78689)
									if (iLocal_74 == 0)
										if (!func_82(14))
											func_43();
										else if (Global_2694571)
											func_42();
								else if (IS_BIT_SET(Global_8372, 9))
									func_41();
								else
									func_24();
					
						if (IS_BIT_SET(Global_8370, 9))
						{
							func_22(0, 0, 1, true);
						}
						else if (Global_22904 == 0)
						{
							if (!IS_BIT_SET(Global_4542302, 3))
							{
								if (Global_78689)
									func_21();
							
								func_20();
								func_18();
							}
						}
						else
						{
							if (Global_78689)
								func_21();
						
							if (!IS_BIT_SET(Global_4542302, 3))
							{
								if (Global_22904 == 1)
								{
									if (IS_BIT_SET(Global_4542302, 1))
									{
										if (IS_BIT_SET(Global_8370, 14))
										{
											if (Global_78689)
												func_22(7, 0, 1, false);
											else
												Global_22905 = 0;
										
											Global_22904 = 0;
											unk_0x8744D2E3FC95740E(&Global_4542302, 1);
										}
									}
									else if (IS_BIT_SET(Global_8370, 14))
									{
										func_22(7, 0, 1, false);
										Global_22904 = 0;
									}
								}
								else
								{
									if (Global_22904 == 3)
									{
										func_22(1, 0, 1, false);
										Global_22904 = 0;
									}
								
									if (Global_22904 == 2)
									{
										func_22(14, 0, 1, false);
										Global_22904 = 0;
									}
								
									if (Global_22904 == 4)
									{
										func_22(23, 0, 1, false);
										Global_22904 = 0;
									}
								}
							}
						}
					}
				}
			}
		}
		else if (unk_0xA6E4F7A73ABC4A76(&(Global_8377[Global_20503 /*15*/].f_5)))
		{
			if (Global_20503 == 0)
			{
				if (unk_0x486FF5D06E9659F1(Global_8377[Global_20503 /*15*/].f_9) == 0)
					Global_20498 = SYSTEM::START_NEW_SCRIPT(&(Global_8377[Global_20503 /*15*/].f_5), CONTACTS_APP);
			}
			else
			{
				num4 = 0;
			
				if (Global_20503 == 23)
					IS_BIT_SET(Global_4542302, 4) == false && Global_1836104 == false;
			
				if (Global_20503 == 2 || num4 == 1)
					if (unk_0x486FF5D06E9659F1(Global_8377[Global_20503 /*15*/].f_9) == 0)
						Global_20498 = SYSTEM::START_NEW_SCRIPT(&(Global_8377[Global_20503 /*15*/].f_5), APP_INTERNET);
				else if (unk_0x486FF5D06E9659F1(Global_8377[Global_20503 /*15*/].f_9) == 0)
					Global_20498 = SYSTEM::START_NEW_SCRIPT(&(Global_8377[Global_20503 /*15*/].f_5), CELLPHONE);
			}
		
			unk_0xFD49725F3FE7EE13(&(Global_8377[Global_20503 /*15*/].f_5));
			Global_8373 = 99;
			unk_0x8744D2E3FC95740E(&Global_8370, 23);
		}
	
		if (unk_0xAE231F549813BBDF(2))
			if (unk_0x4D9174D8796EA622())
				unk_0x66EFB3D6110055C4(2, 200, 1);
	
		if (Global_20500.f_1 == 1)
			func_9();
	
		if (Global_20500.f_1 == 0)
			func_9();
	
		if (Global_20500.f_1 == 3)
		{
			func_9();
		}
		else
		{
			if (Global_21858)
			{
				if (Global_20500.f_1 == 9)
				{
					if (Global_78689)
					{
						if (!IS_BIT_SET(Global_8371, 31))
						{
							if (!IS_BIT_SET(Global_8371, 27))
							{
								if (func_8())
								{
									if (!IS_BIT_SET(Global_8370, 9))
									{
										if (func_62(2, Global_20472, 0))
										{
											if (!unk_0x761778199FE1211C())
											{
												if (!(Global_20499 == 1))
												{
													if (Global_20500.f_1 > 6)
													{
														unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 24);
														unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 27);
														unk_0x8744D2E3FC95740E(&Global_8371, 26);
														unk_0x8744D2E3FC95740E(&Global_8371, 25);
														unk_0x0B0C9A0F9AAEB7F0(&(Global_2794162.f_1832), 17);
													}
												}
											}
										}
									}
								}
							}
						}
					}
				}
			}
		
			if (Global_20483 == 1)
			{
				Global_20485 = 1;
				func_65(0);
			}
		
			if (Global_20484 == 1)
			{
				Global_20485 = 1;
				func_65(0);
			}
		
			if (Global_78689)
			{
				if (!IS_BIT_SET(Global_8372, 0))
				{
					if (unk_0x4D9174D8796EA622() && !func_7())
					{
						if (Global_2801145.f_1 == true)
						{
						}
						else
						{
							func_68();
							func_65(0);
						}
					}
				}
			}
		
			if (unk_0x75EAB09F5E974116(unk_0x259BE71D8A81D4FA()))
			{
				if (unk_0x69799E0840A34AFB(unk_0x4A8C381C258A124D()))
				{
					if (unk_0x837FF265C016E3B7(unk_0x4A8C381C258A124D()) > 0.3f)
						unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 4);
				
					if (unk_0x837FF265C016E3B7(unk_0x4A8C381C258A124D()) == 1f)
					{
						if (IS_BIT_SET(Global_4718592.f_37, 17) && unk_0x834C960822A4683F() || Global_1927335)
						{
							func_5(true, true);
							unk_0x0B0C9A0F9AAEB7F0(&Global_8370, 14);
						}
						else
						{
							Global_20485 = 1;
							func_68();
							func_65(0);
						}
					}
				}
			
				if (func_82(14))
					if (unk_0x4B423FAA24E8ABF0(unk_0x4A8C381C258A124D()) == Global_113810.f_28053[0 /*29*/] || unk_0x4B423FAA24E8ABF0(unk_0x4A8C381C258A124D()) == Global_113810.f_28053[1 /*29*/] || unk_0x4B423FAA24E8ABF0(unk_0x4A8C381C258A124D()) == Global_113810.f_28053[2 /*29*/])
						bLocal_122 = false;
					else if (unk_0x7F420695E3F776FB(unk_0x4A8C381C258A124D(), 0))
						bLocal_122 = true;
			
				if (unk_0x17CB7D9FBEC4100F(unk_0x4A8C381C258A124D()) || unk_0x94CC17D69C81292D(unk_0x259BE71D8A81D4FA()) || unk_0x1BAABBB7277FCD90(unk_0x4A8C381C258A124D()) || unk_0xF6D8C7059C3C2495() || unk_0xB96BD3A32937D44E(unk_0x259BE71D8A81D4FA()) || unk_0x49CB27F641DC1328(unk_0x4A8C381C258A124D()) || unk_0xD6179D448A06A77F(unk_0x4A8C381C258A124D(), 0) || unk_0xC3174723CF144560(unk_0x4A8C381C258A124D()) || unk_0x486FF5D06E9659F1(joaat("michael1")) > 0 || Global_78951 == 1 || func_3(unk_0x4A8C381C258A124D()) || unk_0xD9F5E1FEFD1329E4(unk_0x31945A289F1359A1(unk_0x4A8C381C258A124D()), 0) || unk_0xFFAC548682B3D56E(unk_0x4A8C381C258A124D()) || unk_0xDC7D2B438B687A13() || bLocal_122)
					if (!Global_20443)
						unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 4);
			
				if (unk_0x761778199FE1211C())
				{
					if (Global_78689 == false)
					{
						if (iLocal_104 == 0)
						{
							if (unk_0x7F420695E3F776FB(unk_0x4A8C381C258A124D(), 0))
							{
								if (!unk_0xF4244288C3EF3306(unk_0x4A8C381C258A124D(), unk_0x6EF03BE64E058E2F(unk_0x4A8C381C258A124D(), 0)))
								{
									uLocal_102 = unk_0x1DD05E817C89C737();
									iLocal_104 = 1;
								}
							}
						}
						else
						{
							iLocal_103 = unk_0x1DD05E817C89C737();
							iLocal_106 = iLocal_103 - uLocal_102;
						
							if (iLocal_106 < 4000)
								unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 4);
							else
								iLocal_104 = 0;
						}
					}
				}
			
				if (unk_0x6D05C5731A838CB3(0, 25) || unk_0x6D05C5731A838CB3(0, 68))
				{
					if (Global_78689)
					{
						unk_0x23B29877D0BE9547(unk_0x4A8C381C258A124D(), &iLocal_112, 1);
					
						if (iLocal_112 != joaat("WEAPON_UNARMED") && Global_20500.f_1 < 7)
							unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 4);
					}
				}
			
				if (unk_0x7F420695E3F776FB(unk_0x4A8C381C258A124D(), 0))
				{
					uLocal_113 = unk_0x6EF03BE64E058E2F(unk_0x4A8C381C258A124D(), 0);
				
					if (unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("submersible") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("submersible2"))
						bLocal_116 = true;
					else
						bLocal_116 = false;
				
					if (unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("rhino") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("cutter") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("chernobog") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("khanjali") || bLocal_116 || unk_0xD9F5E1FEFD1329E4(uLocal_113, 0) == 0)
						unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 4);
				
					if (unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("valkyrie") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("insurgent") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("technical") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("trash") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("trash2") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("limo2") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("insurgent3") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("halftrack") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("dune3") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("trailersmall2") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("technical2") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("boxville5") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("mogul") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("bombushka") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("tula") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("chernobog"))
					{
						if (unk_0xF4244288C3EF3306(unk_0x4A8C381C258A124D(), uLocal_113))
						{
							num5 = func_2(unk_0x4A8C381C258A124D(), uLocal_113);
						
							if (num5 != -2)
							{
								if (unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("insurgent") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("limo2"))
									if (num5 == 3)
										unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 4);
							
								if (unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("trash") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("trash2"))
									if (num5 == 2 || num5 == 1)
										unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 4);
							
								if (unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("technical2"))
									if (num5 == 1)
										unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 4);
							
								if (unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("boxville5"))
									if (num5 == 3)
										unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 4);
							
								if (unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("insurgent3"))
									if (num5 == 3)
										unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 4);
							
								if (unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("halftrack"))
									if (num5 == 1)
										unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 4);
							
								if (unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("dune3"))
									if (num5 == 0)
										unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 4);
							
								if (unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("trailersmall2"))
									if (num5 == -1)
										unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 4);
							
								if (unk_0x333A2A6253C809D2(uLocal_113, num5))
									unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 4);
							}
						}
					}
				
					if (unk_0x875A214D5EBCA509(0, 69))
					{
						if (Global_78689 == false)
						{
							if (Global_20500.f_1 == 6 || Global_20500.f_1 == 7)
							{
								flag = true;
							
								if (unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("stromberg") && unk_0x69799E0840A34AFB(uLocal_113))
									flag = false;
							
								if (unk_0xBA16CD57E37AC32A(unk_0x4B423FAA24E8ABF0(uLocal_113)) || unk_0x00C6FDED3EB75117(unk_0x4B423FAA24E8ABF0(uLocal_113)) || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("submersible") || unk_0x4B423FAA24E8ABF0(uLocal_113) == joaat("submersible2"))
									flag = false;
							
								if (flag)
									func_65(0);
							}
						}
					}
				}
				else
				{
					if (unk_0x875A214D5EBCA509(0, 24))
						if (Global_78689 == false)
							if (Global_20500.f_1 == 6 || Global_20500.f_1 == 7)
								func_65(0);
				
					bLocal_116 = false;
				}
			
				if (unk_0x9AC89B274C35B3FC(unk_0x4A8C381C258A124D()))
				{
					if (IS_BIT_SET(Global_4718592.f_37, 17) && unk_0x834C960822A4683F() || Global_1927335)
					{
						func_5(true, true);
					}
					else
					{
						Global_20485 = 1;
						func_68();
						func_65(0);
					}
				}
			
				if (Global_78689 == false)
				{
					if (unk_0x8D91ADE44AC79BC9(unk_0x4A8C381C258A124D()) < uLocal_99)
					{
						if (!unk_0x7F420695E3F776FB(unk_0x4A8C381C258A124D(), 0))
						{
							Global_20485 = 1;
							func_68();
							func_65(0);
						}
					}
				
					if (Global_20500.f_1 == 9 || Global_20500.f_1 == 10)
					{
						if (unk_0x9AC89B274C35B3FC(unk_0x4A8C381C258A124D()))
						{
							Global_20485 = 1;
							func_68();
							func_65(0);
						}
					}
				}
				else if (func_75())
				{
					if (unk_0x8D91ADE44AC79BC9(unk_0x4A8C381C258A124D()) < uLocal_99)
					{
						if (!unk_0x7F420695E3F776FB(unk_0x4A8C381C258A124D(), 0))
						{
							Global_20485 = 1;
							func_68();
							func_65(0);
						}
					}
				}
			
				if (unk_0x196704C916969409())
				{
					if (!unk_0xD6179D448A06A77F(unk_0x4A8C381C258A124D(), 1))
					{
						if (!func_75())
						{
							unk_0x23B29877D0BE9547(unk_0x4A8C381C258A124D(), &iLocal_112, 1);
						
							if (iLocal_112 == joaat("WEAPON_SNIPERRIFLE") || iLocal_112 == joaat("WEAPON_HEAVYSNIPER") || iLocal_112 == joaat("WEAPON_REMOTESNIPER"))
								bLocal_111 = true;
							else
								bLocal_111 = false;
						
							if (unk_0x5EA7A06A1491D450())
							{
								if (bLocal_111)
								{
									Global_20485 = 1;
									func_68();
									func_65(0);
								}
							}
						}
					}
				}
			
				if (unk_0xA6534E6C7AE314D6(unk_0x4A8C381C258A124D()))
				{
					Global_20485 = 1;
					func_68();
					func_65(0);
				}
			
				if (unk_0x11BF418D1AFD7970(unk_0x4A8C381C258A124D(), 0))
				{
					Global_20485 = 1;
					func_68();
					func_65(0);
				}
			
				if (unk_0x6652E84A43F51B8B(unk_0x259BE71D8A81D4FA(), 1))
				{
					Global_20485 = 1;
					func_68();
					func_65(0);
				}
			
				if (unk_0x8BF5256C439DF778(unk_0x4A8C381C258A124D()) || unk_0x333554781C7582D7(unk_0x4A8C381C258A124D()))
				{
					if (Global_78689 == false)
					{
						Global_20485 = 1;
						func_68();
						func_65(0);
					}
				}
			}
			else
			{
				num6 = 0;
			
				if (Global_78689 && Global_2801145.f_1 && Global_2801145.f_37 && Global_20500.f_1 == 9)
				{
					num6 = 1;
				
					if (!IS_BIT_SET(Global_8371, 24))
					{
						if (IS_BIT_SET(Global_8371, 26))
						{
							unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 25);
							unk_0x8744D2E3FC95740E(&Global_8371, 26);
							unk_0x8744D2E3FC95740E(&Global_8372, 2);
						}
					}
				}
			
				if (num6 == 0)
				{
					Global_20485 = 1;
					func_68();
					func_65(0);
				}
			}
		}
	
		if (Global_20478 == 1)
			func_1();
	
		if (Global_20499 == 2)
		{
			if (unk_0xAE231F549813BBDF(2))
			{
				unk_0x4CC1CF98851922CE(0, Global_20469);
				unk_0x4CC1CF98851922CE(0, Global_20470);
				unk_0x66EFB3D6110055C4(0, 24, 1);
				unk_0x66EFB3D6110055C4(0, 257, 1);
			}
		
			if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
				unk_0x92187CF73A8D1B96(unk_0x4A8C381C258A124D());
		}
	}

	return;
}

void func_1() // Position - 0x1ECB
{
	if (!unk_0x6D05C5731A838CB3(2, Global_20469) && !unk_0x6D05C5731A838CB3(2, Global_20468))
		Global_20478 = 0;

	return;
}

int func_2(var uParam0, var uParam1) // Position - 0x1EF1
{
	if (!unk_0x1C2F771CDC87A3A5(uParam0, 0) && !unk_0x1C2F771CDC87A3A5(uParam1, 0))
	{
		if (unk_0xF4244288C3EF3306(uParam0, uParam1))
		{
			if (unk_0xFD5C5BBD1FE92BB7(uParam1, -1, 0) == uParam0)
				return -1;
		
			if (unk_0xFD5C5BBD1FE92BB7(uParam1, 0, 0) == uParam0)
				return 0;
		
			if (unk_0xFD5C5BBD1FE92BB7(uParam1, 1, 0) == uParam0)
				return 1;
		
			if (unk_0xFD5C5BBD1FE92BB7(uParam1, 2, 0) == uParam0)
				return 2;
		
			if (unk_0xFD5C5BBD1FE92BB7(uParam1, 3, 0) == uParam0)
				return 3;
		
			if (unk_0xFD5C5BBD1FE92BB7(uParam1, 4, 0) == uParam0)
				return 4;
		
			if (unk_0xFD5C5BBD1FE92BB7(uParam1, 5, 0) == uParam0)
				return 5;
		
			if (unk_0xFD5C5BBD1FE92BB7(uParam1, 6, 0) == uParam0)
				return 6;
		
			if (unk_0xFD5C5BBD1FE92BB7(uParam1, 7, 0) == uParam0)
				return 3;
		
			if (unk_0xFD5C5BBD1FE92BB7(uParam1, 8, 0) == uParam0)
				return 4;
		}
	}

	return -2;
}

BOOL func_3(int iParam0) // Position - 0x1FCA
{
	if (iParam0 == 0)
		return false;

	if (func_4(iParam0) == -1)
		return false;

	return true;
}

int func_4(int iParam0) // Position - 0x1FEB
{
	int i;

	if (iParam0 == 0)
		return -1;

	i = 0;

	for (i = 0; i < 16; i = i + 1)
	{
		if (!(Global_44355[i /*5*/] == -1))
			if (iParam0 == Global_44355[i /*5*/].f_1)
				return i;
	}

	return -1;
}

void func_5(BOOL bParam0, BOOL bParam1) // Position - 0x2034
{
	if (bParam0)
	{
		if (func_6(0))
		{
			Global_20704 = true;
		
			if (bParam1)
				unk_0x78C4EBB0251847E2(&Global_20437);
		
			Global_20428 = { Global_20446[Global_20445 /*3*/] };
			unk_0x6B7EA0158D00C600(Global_20428);
		}
	}
	else if (Global_20704 == true)
	{
		Global_20704 = false;
		Global_20428 = { Global_20453[Global_20445 /*3*/] };
	
		if (bParam1)
			unk_0x6B7EA0158D00C600(Global_20437);
		else
			unk_0x6B7EA0158D00C600(Global_20428);
	}

	return;
}

BOOL func_6(int iParam0) // Position - 0x20A8
{
	if (iParam0 == 1)
		if (Global_20500.f_1 > 3)
			if (IS_BIT_SET(Global_8370, 14))
				return true;
			else
				return false;
		else
			return false;

	if (unk_0x486FF5D06E9659F1(joaat("cellphone_flashhand")) > 0)
		return true;

	if (Global_20500.f_1 > 3)
		return true;

	return false;
}

BOOL func_7() // Position - 0x20FF
{
	return IS_BIT_SET(Global_1963795, 5);
}

BOOL func_8() // Position - 0x210D
{
	return Global_2801145.f_1;
}

void func_9() // Position - 0x211B
{
	float num;
	var unk;
	float num2;

	Global_22904 = 0;
	Global_22905 = 0;
	unk_0x4CC1CF98851922CE(0, Global_20470);
	unk_0x4EB223432F8FA0A0(6);
	unk_0x4EB223432F8FA0A0(7);
	unk_0x4EB223432F8FA0A0(8);
	unk_0x4EB223432F8FA0A0(9);

	if (Global_20466 == 0)
	{
		unk_0xBA290C9390813C9F(500f);
	
		if (func_74())
			iLocal_16 = unk_0x1DD05E817C89C737();
		else
			iLocal_16 = 0;
	
		unk_0xFC75D88023832ED9(1);
		Global_20466 = 1;
	}

	if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
		if (unk_0x129DE7ED6456C4CA(unk_0x4A8C381C258A124D()))
			unk_0x92187CF73A8D1B96(unk_0x4A8C381C258A124D());

	if (unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
		if (unk_0x129DE7ED6456C4CA(unk_0x4A8C381C258A124D()))
			unk_0x92187CF73A8D1B96(unk_0x4A8C381C258A124D());

	if (Global_113810.f_14054.f_84 == 1)
	{
		Global_113810.f_14054.f_84 = 0;
		unk_0x665A7E873A6664BC(0, 1);
	}

	num = 350f;

	if (IS_BIT_SET(Global_8370, 30) || IS_BIT_SET(Global_8372, 2))
		num = 25f;

	unk = { Global_20460 };

	if (Global_20485 == 1)
		unk = { -45f, 45f, 25f };

	if (IS_BIT_SET(Global_8371, 26) || IS_BIT_SET(Global_8370, 30) || IS_BIT_SET(Global_8372, 2))
		uLocal_118 = { Global_20446[Global_20445 /*3*/] };
	else
		uLocal_118 = { Global_20453[Global_20445 /*3*/] };

	num2 = func_15(uLocal_118, Global_20446[Global_20445 /*3*/], Global_20460, unk, num, false);

	if (!bLocal_86 && num2 >= 1f)
	{
		unk_0x5F2E2CEDE90F627E();
		bLocal_86 = true;
	}

	if (bLocal_86 && unk_0x1DD05E817C89C737() - iLocal_16 > 500)
	{
		unk_0x4CC1CF98851922CE(0, Global_20470);
		Global_20500.f_1 == 3;
		Global_20500.f_1 == 1;
		Global_20500.f_1 == 0;
		unk_0xFC75D88023832ED9(0);
		Global_20441 = 0;
		Global_20482 = false;
		Global_20702 = 0;
	
		if (Global_113810.f_14054.f_84 == 1)
		{
			Global_113810.f_14054.f_84 = 0;
			unk_0x665A7E873A6664BC(0, 1);
		}
	
		Global_20444 = 1;
	
		if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
			unk_0x92187CF73A8D1B96(unk_0x4A8C381C258A124D());
	
		Global_22859 == 0;
		Global_20704 = false;
	
		if (func_12(0))
			func_11();
	
		unk_0x8744D2E3FC95740E(&Global_8371, 8);
		unk_0x8744D2E3FC95740E(&Global_8370, 14);
		unk_0x8744D2E3FC95740E(&Global_8370, 9);
		unk_0x8744D2E3FC95740E(&Global_8370, 27);
		unk_0x8744D2E3FC95740E(&Global_8370, 30);
		unk_0x8744D2E3FC95740E(&Global_8371, 5);
		unk_0x8744D2E3FC95740E(&Global_8371, 19);
		unk_0x8744D2E3FC95740E(&Global_8371, 21);
		unk_0x8744D2E3FC95740E(&Global_8371, 24);
		unk_0x8744D2E3FC95740E(&Global_8371, 25);
		unk_0x8744D2E3FC95740E(&Global_8371, 27);
		unk_0x8744D2E3FC95740E(&Global_8371, 26);
		unk_0x8744D2E3FC95740E(&Global_8372, 2);
		Global_2804775 = 0;
		iLocal_121 = 0;
	
		if (!unk_0x75EAB09F5E974116(unk_0x259BE71D8A81D4FA()))
			func_10();
	
		SYSTEM::SETTIMERA(0);
	
		if (unk_0x486FF5D06E9659F1(Global_8377[2 /*15*/].f_9) == 0)
		{
			while (unk_0xF40767E41852FB72(Global_20498))
			{
				SYSTEM::WAIT(0);
				unk_0x4CC1CF98851922CE(0, Global_20470);
			
				if (unk_0xA0C7B98BCF1EEF9E(Global_20481))
				{
					unk_0x61055542D9D877AE(iLocal_85);
					unk_0x6D16B99FEB0AFFF1(Global_20481, Global_20420, Global_20421, Global_20422, Global_20423, 255, 255, 255, 255, 0);
				}
			
				if (SYSTEM::TIMERA() > 5000)
					unk_0xC3CF96F73FC43848(Global_20498);
			}
		}
	
		Global_20488 = false;
	
		if (unk_0xA0C7B98BCF1EEF9E(Global_20481))
			func_120(Global_20481, "SHUTDOWN_MOVIE");
	
		SYSTEM::WAIT(0);
		unk_0xD314260005F064BF(&Global_20481);
		Global_20466 = 0;
		unk_0x4CC1CF98851922CE(0, Global_20470);
	
		if (unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
			if (Global_2801145.f_1)
				if (Global_78689)
					unk_0xBF3D28CA44F3BE2D(-1, "Hang_Up", "Phone_SoundSet_Michael", 1);
				else
					unk_0xBF3D28CA44F3BE2D(-1, "Hang_Up", &Global_20489, 1);
	
		func_117();
		func_115();
		Global_21858 = false;
		Global_4542581 = 0;
		unk_0xBBC29EBE6E1A48FA();
	}

	return;
}

void func_10() // Position - 0x2494
{
	unk_0xC78B293A5F4EACF9();
	Global_22856 = 0;

	if (unk_0xE87F28FD4128D063() || Global_20500.f_1 == 9 || Global_20499 == 1)
	{
		unk_0x0F15249D24BC5ADA(0);
		Global_21845 = 6;
		Global_20500.f_1 = 3;
		return;
	}

	if (unk_0x1F9EB85925C3ECD7())
	{
		unk_0x0F15249D24BC5ADA(1);
		Global_21845 = 6;
		return;
	}

	return;
}

void func_11() // Position - 0x24EB
{
	if (Global_9175[0 /*2811*/][0 /*281*/].f_259 == 2)
		Global_9175[0 /*2811*/][0 /*281*/].f_259 = 0;

	if (Global_9175[1 /*2811*/][0 /*281*/].f_259 == 2)
		Global_9175[1 /*2811*/][0 /*281*/].f_259 = 0;

	if (Global_9175[2 /*2811*/][0 /*281*/].f_259 == 2)
		Global_9175[2 /*2811*/][0 /*281*/].f_259 = 0;

	unk_0x8744D2E3FC95740E(&Global_8370, 25);
	unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 11);
	return;
}

BOOL func_12(int iParam0) // Position - 0x2568
{
	if (Global_43377 == 15)
		return false;

	if (func_13(iParam0))
		return false;

	return true;
}

BOOL func_13(int iParam0) // Position - 0x258A
{
	return func_14(iParam0, Global_43377);
}

BOOL func_14(int iParam0, int iParam1) // Position - 0x259B
{
	if (iParam1 == 15)
		return 1;

	if (iParam0 == 15)
		return 0;

	switch (iParam0)
	{
		case 16:
			switch (iParam1)
			{
				case 9:
				case 10:
				case 7:
				case 13:
				case 14:
					return 0;
			}
		
			return 1;
	
		case 0:
			switch (iParam1)
			{
				case 5:
				case 17:
					return 1;
			}
			break;
	
		case 2:
		case 3:
			switch (iParam1)
			{
				case 5:
				case 6:
				case 8:
				case 17:
					return 1;
			}
			break;
	
		case 4:
			if (iParam1 == 17)
				return 1;
			break;
	
		case 5:
			break;
	
		case 6:
		case 8:
			if (iParam1 == 5)
				return 1;
			break;
	
		case 7:
			if (iParam1 == 6)
				return 1;
			break;
	
		case 9:
			if (iParam1 == 5)
				return 1;
			break;
	
		case 10:
			switch (iParam1)
			{
				case 5:
				case 6:
				case 17:
					return 1;
			}
			break;
	
		case 11:
			if (iParam1 == 5)
				return 1;
			break;
	
		case 17:
			switch (iParam1)
			{
				case 17:
				case 12:
				case 5:
					return 1;
			}
			break;
	
		case 18:
		case 12:
			switch (iParam1)
			{
				case 5:
				case 6:
				case 8:
					return 1;
			}
			break;
	
		case 13:
			switch (iParam1)
			{
				case 5:
					return 1;
			}
			break;
	
		case 14:
			switch (iParam1)
			{
				case 5:
					return 1;
			}
			break;
	}

	return 0;
}

float func_15(var uParam0, var uParam1, var uParam2, var uParam3, var uParam4, var uParam5, float fParam6, float fParam7, float fParam8, float fParam9, float fParam10, float fParam11, float fParam12, BOOL bParam13) // Position - 0x277C
{
	var unk;
	float num;
	float num2;
	float num3;

	if (Global_4542581 == 0)
	{
		if (IS_BIT_SET(Global_8370, 14) && Global_20500.f_1 < 4)
		{
			unk_0x78C4EBB0251847E2(&unk);
		
			if (Global_20453[Global_20445 /*3*/].f_1 == unk.f_1)
				Global_4542581 = 1;
		}
	}

	if (func_74() && Global_4542581 == 0)
		return 2f;

	if (iLocal_16 == 0)
		iLocal_16 = unk_0x1DD05E817C89C737();

	num = func_17(SYSTEM::TO_FLOAT(unk_0x1DD05E817C89C737() - iLocal_16) / fParam12, 0f, 1f);

	if (num < 1f)
	{
		num2 = num;
	
		if (bParam13)
		{
			num2 = num2 - 1f;
			num3 = 0.670158f;
			num2 = (num2 * num2 * (((num3 + 1f) * num2) + num3)) + 1f;
		}
		else
		{
			num2 = SYSTEM::SIN(num * 90f);
		}
	
		Global_20428 = { func_16(uParam0, uParam3, num2) };
		Global_20431 = { func_16(fParam6, fParam9, num2) };
	}
	else
	{
		Global_20428 = { uParam3 };
		Global_20431 = { fParam9 };
	}

	unk_0x6B7EA0158D00C600(Global_20428);
	unk_0xA0382000AA987093(Global_20431, 0);
	return num;
}

Vector3 func_16(var uParam0, var uParam1, var uParam2, var uParam3, var uParam4, var uParam5, float fParam6) // Position - 0x288B
{
	return uParam0 + ((uParam3 - uParam0) * { fParam6, fParam6, fParam6 });
}

float func_17(float fParam0, float fParam1, float fParam2) // Position - 0x28A5
{
	if (fParam0 > fParam2)
		return fParam2;
	else if (fParam0 < fParam1)
		return fParam1;

	return fParam0;
}

void func_18() // Position - 0x28CC
{
	if (Global_20478 == 0)
	{
		if (func_62(2, Global_20472, 0))
		{
			if (Global_78689 == false)
			{
				if (Global_20706)
				{
					if (Global_20705 == false)
					{
						Global_20705 = true;
						func_113(Global_20481, "SET_DATA_SLOT_EMPTY", 1f, -1082130432, -1082130432, -1082130432, -1082130432);
						func_83(2);
						func_19();
						func_113(Global_20481, "DISPLAY_VIEW", 1f, SYSTEM::TO_FLOAT(Global_20505), -1082130432, -1082130432, -1082130432);
						Global_20480 = Global_20505;
					}
					else
					{
						Global_20705 = false;
						func_113(Global_20481, "SET_DATA_SLOT_EMPTY", 1f, -1082130432, -1082130432, -1082130432, -1082130432);
						func_83(1);
						func_19();
						func_113(Global_20481, "DISPLAY_VIEW", 1f, SYSTEM::TO_FLOAT(Global_20504), -1082130432, -1082130432, -1082130432);
						Global_20480 = Global_20504;
					}
				}
			}
		}
	}

	return;
}

void func_19() // Position - 0x29A5
{
	if (Global_20706 == false)
	{
		func_81(Global_20481, "SET_SOFT_KEYS", 1f, 0f, 1f, -1f, -1f, 0, 0, 0, 0, 0);
		unk_0x8744D2E3FC95740E(&Global_8370, 17);
	}
	else if (Global_78689)
	{
		func_81(Global_20481, "SET_SOFT_KEYS", 1f, 0f, 1f, -1f, -1f, 0, 0, 0, 0, 0);
		unk_0x8744D2E3FC95740E(&Global_8370, 17);
	}
	else
	{
		if (Global_20705 == true)
			if (Global_20488)
				func_81(Global_20481, "SET_SOFT_KEYS", 1f, 1f, 20f, -1f, -1f, "CELL_225" /*NEXT*/, 0, 0, 0, 0);
			else
				func_81(Global_20481, "SET_SOFT_KEYS", 1f, 1f, 20f, -1f, -1f, 0, 0, 0, 0, 0);
		else if (Global_20488)
			func_81(Global_20481, "SET_SOFT_KEYS", 1f, 1f, 3f, -1f, -1f, "CELL_225" /*NEXT*/, 0, 0, 0, 0);
		else
			func_81(Global_20481, "SET_SOFT_KEYS", 1f, 1f, 3f, -1f, -1f, 0, 0, 0, 0, 0);
	
		unk_0x0B0C9A0F9AAEB7F0(&Global_8370, 17);
	}

	return;
}

void func_20() // Position - 0x2A7E
{
	if (IS_BIT_SET(Global_8371, 10) || iLocal_121 == 1)
	{
		Global_8977 = unk_0x1DD05E817C89C737();
		Global_8976 = 0;
		Global_20478 = 1;
		unk_0x88F483FBD433696A(Global_20481, "GET_CURRENT_SELECTION");
		uLocal_72 = unk_0x6CA3F2B87712B6A4();
		bLocal_73 = true;
	}
	else if (Global_20478 == 0)
	{
		if (func_62(2, Global_20469, 0))
		{
			if (iLocal_74 == 0)
			{
				Global_8977 = unk_0x1DD05E817C89C737();
				Global_8976 = 0;
				Global_20478 = 1;
				unk_0x88F483FBD433696A(Global_20481, "GET_CURRENT_SELECTION");
				uLocal_72 = unk_0x6CA3F2B87712B6A4();
				bLocal_73 = true;
			}
		}
	}

	return;
}

void func_21() // Position - 0x2AFD
{
	if (Global_20478 == 0)
	{
		if (func_62(2, Global_20467, 1))
		{
			if (func_107(2092, -1) == 1)
			{
				if (Global_113810.f_14054[Global_20500 /*20*/].f_17 == 0)
				{
					if (!IS_BIT_SET(Global_8371, 3))
					{
						if (!Global_20443)
						{
							if (!IS_BIT_SET(Global_4542302, 3))
							{
								if (!IS_BIT_SET(Global_8370, 14))
								{
									Global_20478 = 1;
									unk_0x0B0C9A0F9AAEB7F0(&Global_4542302, 3);
									func_22(3, 0, 1, false);
									Global_22904 = 0;
								}
							}
						}
					}
				}
			}
		}
	}

	return;
}

int func_22(int iParam0, int iParam1, int iParam2, BOOL bParam3) // Position - 0x2B74
{
	func_123();

	if (Global_78689 == false)
	{
		if (func_82(14))
		{
			if (Global_22904 == 2 || Global_22904 == 4)
			{
			}
			else
			{
				return 0;
			}
		}
	}

	if (Global_20500.f_1 == 9)
		return 0;

	if (iParam2 == 0)
		if (func_6(0) == true)
			return 0;

	if (Global_20466 == 1)
		return 0;

	if (Global_20500.f_1 == 7)
		return 0;

	iParam1 == 1;

	if (!unk_0xF40767E41852FB72(Global_20497))
	{
		if (unk_0x486FF5D06E9659F1(joaat("cellphone_flashhand")) == 0)
		{
			if (Global_20500.f_1 < 4)
			{
				func_23("cellphone_flashhand");
			
				if (unk_0x486FF5D06E9659F1(joaat("cellphone_flashhand")) == 0)
					Global_20497 = SYSTEM::START_NEW_SCRIPT("cellphone_flashhand", DEFAULT);
			
				unk_0xFD49725F3FE7EE13("cellphone_flashhand");
			}
		}
	}

	while (!Global_20482)
	{
		SYSTEM::WAIT(0);
	}

	func_85();
	func_84();

	if (unk_0x486FF5D06E9659F1(Global_8377[iParam0 /*15*/].f_9) == 0)
	{
		Global_8976 = 0;
		Global_20500.f_1 = 7;
		func_23(&(Global_8377[iParam0 /*15*/].f_5));
	
		if (bParam3)
			if (unk_0x486FF5D06E9659F1(Global_8377[iParam0 /*15*/].f_9) == 0)
				Global_20498 = SYSTEM::START_NEW_SCRIPT(&(Global_8377[iParam0 /*15*/].f_5), CONTACTS_APP);
		else if (unk_0x486FF5D06E9659F1(Global_8377[iParam0 /*15*/].f_9) == 0)
			Global_20498 = SYSTEM::START_NEW_SCRIPT(&(Global_8377[iParam0 /*15*/].f_5), CELLPHONE);
	
		unk_0xFD49725F3FE7EE13(&(Global_8377[iParam0 /*15*/].f_5));
		return 1;
	}

	return 1;
}

void func_23(char* sParam0) // Position - 0x2CDA
{
	unk_0x97A5024CE91641F1(sParam0);

	while (!unk_0xA6E4F7A73ABC4A76(sParam0))
	{
		SYSTEM::WAIT(0);
	}

	return;
}

void func_24() // Position - 0x2CFA
{
	if (func_40())
	{
		if (func_62(2, Global_20472, 0))
		{
			func_38();
			Global_20504 = Global_20480;
			func_35();
			return;
		}
	}

	if (Global_20479)
		if (SYSTEM::TIMERA() > 50)
			Global_20479 = false;

	if (unk_0xAE231F549813BBDF(2))
	{
		if (unk_0x875A214D5EBCA509(2, 180))
		{
			switch (Global_20480)
			{
				case 0:
					if (Global_8939[1])
					{
						func_33();
						Global_20480 = 1;
					}
					break;
			
				case 1:
					if (Global_8939[2])
					{
						func_33();
						Global_20480 = 2;
					}
					break;
			
				case 2:
					if (Global_8939[3])
					{
						func_33();
						func_31();
						Global_20480 = 3;
					}
					else
					{
						func_29();
						func_31();
						Global_20480 = 4;
					}
					break;
			
				case 3:
					if (Global_8939[4])
					{
						func_33();
						Global_20480 = 4;
					}
					break;
			
				case 4:
					if (Global_8939[5])
					{
						func_33();
						Global_20480 = 5;
					}
					break;
			
				case 5:
					if (Global_8939[6])
					{
						func_33();
						func_31();
						Global_20480 = 6;
					}
					break;
			
				case 6:
					if (Global_8939[7])
					{
						func_33();
						Global_20480 = 7;
					}
					break;
			
				case 7:
					if (Global_8939[8])
					{
						func_33();
						Global_20480 = 8;
					}
					else
					{
						func_29();
						func_31();
						Global_20480 = 0;
					}
					break;
			
				case 8:
					if (Global_8939[0])
					{
						func_33();
						func_31();
						Global_20480 = 0;
					}
					else
					{
						func_29();
						func_31();
					}
					break;
			}
		}
	
		if (unk_0x875A214D5EBCA509(2, 181))
		{
			switch (Global_20480)
			{
				case 0:
					if (Global_8939[8])
					{
						func_29();
						func_26();
						Global_20480 = 8;
					}
					else
					{
						func_33();
						func_26();
						Global_20480 = 7;
					}
					break;
			
				case 1:
					if (Global_8939[0])
					{
						func_29();
						Global_20480 = 0;
					}
					else if (Global_8939[8])
					{
						func_26();
						func_33();
					}
					else
					{
						func_26();
					}
					break;
			
				case 2:
					if (Global_8939[1])
					{
						func_29();
						Global_20480 = 1;
					}
					break;
			
				case 3:
					if (Global_8939[2])
					{
						func_29();
						func_26();
						Global_20480 = 2;
					}
					break;
			
				case 4:
					if (Global_8939[3])
					{
						func_29();
						Global_20480 = 3;
					}
					else
					{
						func_26();
						func_33();
						Global_20709 = 1;
						Global_20480 = 2;
					}
					break;
			
				case 5:
					if (Global_8939[4])
					{
						func_29();
						Global_20480 = 4;
					}
					break;
			
				case 6:
					if (Global_8939[5])
					{
						func_26();
						func_29();
						Global_20480 = 5;
					}
					break;
			
				case 7:
					if (Global_8939[6])
					{
						func_29();
						Global_20480 = 6;
					}
					break;
			
				case 8:
					if (Global_8939[7])
					{
						func_29();
						Global_20480 = 7;
					}
					break;
			}
		}
	}

	if (Global_20479 == false)
	{
		if (func_62(2, Global_20475, 0))
		{
			Global_20709 = 0;
		
			switch (Global_20480)
			{
				case 2:
					if (Global_8939[0] == true)
					{
						Global_20480 = 0;
					}
					else
					{
						Global_20480 = 1;
						Global_20709 = 1;
					}
					break;
			
				case 5:
					Global_20480 = 3;
					break;
			
				case 6:
					Global_20480 = 7;
					break;
			
				case 7:
					if (Global_8939[Global_20480 + 1] == true)
					{
						Global_20480 = 8;
					}
					else
					{
						Global_20480 = 6;
						Global_20709 = 1;
					}
					break;
			
				case 8:
					Global_20480 = 6;
					break;
			
				default:
					Global_20480 = Global_20480 + 1;
					break;
			}
		
			if (Global_20709 == 1)
				func_29();
			else
				func_33();
		
			Global_20479 = true;
			SYSTEM::SETTIMERA(0);
		}
	
		if (func_62(2, Global_20474, 0))
		{
			Global_20709 = 0;
		
			switch (Global_20480)
			{
				case 0:
					Global_20480 = 2;
					break;
			
				case 1:
					if (Global_8939[0] == true)
					{
						Global_20480 = 0;
					}
					else
					{
						Global_20480 = 2;
						Global_20709 = 1;
					}
					break;
			
				case 3:
					Global_20480 = 5;
					break;
			
				case 6:
					if (Global_8939[8] == true)
					{
						Global_20480 = 8;
					}
					else
					{
						Global_20480 = 7;
						Global_20709 = 1;
					}
					break;
			
				default:
					Global_20480 = Global_20480 - 1;
					break;
			}
		
			if (Global_20709 == 1)
				func_33();
			else
				func_29();
		
			Global_20479 = true;
			SYSTEM::SETTIMERA(0);
		}
	
		if (func_62(2, Global_20476, 0))
		{
			Global_20709 = 0;
		
			switch (Global_20480)
			{
				case 0:
					if (Global_8939[6])
						Global_20480 = 6;
					break;
			
				case 1:
					if (Global_8939[7])
					{
						Global_20480 = 7;
					}
					else
					{
						Global_20709 = 1;
						Global_20480 = 4;
					}
					break;
			
				case 2:
					if (Global_8939[8])
					{
						Global_20480 = 8;
					}
					else
					{
						Global_20709 = 1;
						Global_20480 = 5;
					}
					break;
			
				case 3:
					if (Global_8939[0])
					{
						Global_20480 = 0;
					}
					else
					{
						Global_20709 = 1;
						Global_20480 = 6;
					}
					break;
			
				case 4:
					if (Global_8939[1])
						Global_20480 = 1;
					break;
			
				case 5:
					if (Global_8939[2])
						Global_20480 = 2;
					break;
			
				case 6:
					if (Global_8939[3])
						Global_20480 = 3;
					break;
			
				case 7:
					if (Global_8939[4])
						Global_20480 = 4;
					break;
			
				case 8:
					if (Global_8939[5])
						Global_20480 = 5;
					break;
			}
		
			if (Global_20709 == 1)
				func_31();
			else
				func_26();
		
			Global_20479 = true;
			SYSTEM::SETTIMERA(0);
		}
	
		if (func_62(2, Global_20477, 0))
		{
			Global_20709 = 0;
		
			switch (Global_20480)
			{
				case 0:
					if (Global_8939[3])
						Global_20480 = 3;
					break;
			
				case 1:
					if (Global_8939[4])
						Global_20480 = 4;
					break;
			
				case 2:
					if (Global_8939[5])
						Global_20480 = 5;
					break;
			
				case 3:
					if (Global_8939[6])
						Global_20480 = 6;
					break;
			
				case 4:
					if (Global_8939[7])
					{
						Global_20480 = 7;
					}
					else
					{
						Global_20709 = 1;
						Global_20480 = 1;
					}
					break;
			
				case 5:
					if (Global_8939[8])
					{
						Global_20480 = 8;
					}
					else
					{
						Global_20709 = 1;
						Global_20480 = 2;
					}
					break;
			
				case 6:
					if (Global_8939[0])
					{
						Global_20480 = 0;
					}
					else
					{
						Global_20709 = 1;
						Global_20480 = 3;
					}
					break;
			
				case 7:
					if (Global_8939[1])
						Global_20480 = 1;
					break;
			
				case 8:
					if (Global_8939[2])
						Global_20480 = 2;
					break;
			}
		
			if (Global_20709 == 1)
				func_26();
			else
				func_31();
		
			Global_20479 = true;
			SYSTEM::SETTIMERA(0);
		}
	}

	if (Global_8377[23 /*15*/].f_10 == 57)
	{
		if (Global_20479 == true && Global_20480 == 8)
		{
			if (iLocal_123 == 0)
			{
				iLocal_123 = 1;
				func_25(12);
			}
		}
	}

	return;
}

void func_25(int iParam0) // Position - 0x3377
{
	int num;
	int num2;

	num = iParam0 / 32;
	num2 = iParam0 % 32;
	unk_0x0B0C9A0F9AAEB7F0(&Global_2794162.f_5231.f_7[num], num2);
	return;
}

void func_26() // Position - 0x33A0
{
	func_113(Global_20481, "SET_INPUT_EVENT", SYSTEM::TO_FLOAT(1), -1082130432, -1082130432, -1082130432, -1082130432);
	unk_0xBF3D28CA44F3BE2D(-1, "Menu_Navigate", &Global_20489, 1);
	func_27();
	return;
}

void func_27() // Position - 0x33DD
{
	if (func_28())
		if (Global_20709 == 0)
			unk_0xBF3F90E670404C44(1);
		else
			unk_0xBF3F90E670404C44(2);

	return;
}

BOOL func_28() // Position - 0x3400
{
	var unk;
	int num;
	BOOL flag;

	if (Global_78689)
		return false;

	flag = 0;
	unk = unk_0xBF03D0685ADC793B();
	num = unk_0xBCF87EE3DC296C2A(unk);

	if (num == 4)
		flag = 1;

	if (Global_4542580 || flag)
		return true;

	return true;
}

void func_29() // Position - 0x3447
{
	func_113(Global_20481, "SET_INPUT_EVENT", SYSTEM::TO_FLOAT(4), -1082130432, -1082130432, -1082130432, -1082130432);
	unk_0xBF3D28CA44F3BE2D(-1, "Menu_Navigate", &Global_20489, 1);
	func_30();
	return;
}

void func_30() // Position - 0x3484
{
	if (func_28())
		if (Global_20709 == 0)
			unk_0xBF3F90E670404C44(3);
		else
			unk_0xBF3F90E670404C44(4);

	return;
}

void func_31() // Position - 0x34A7
{
	func_113(Global_20481, "SET_INPUT_EVENT", SYSTEM::TO_FLOAT(3), -1082130432, -1082130432, -1082130432, -1082130432);
	unk_0xBF3D28CA44F3BE2D(-1, "Menu_Navigate", &Global_20489, 1);
	func_32();
	return;
}

void func_32() // Position - 0x34E4
{
	if (func_28())
		if (Global_20709 == 0)
			unk_0xBF3F90E670404C44(2);
		else
			unk_0xBF3F90E670404C44(1);

	return;
}

void func_33() // Position - 0x3507
{
	func_113(Global_20481, "SET_INPUT_EVENT", SYSTEM::TO_FLOAT(2), -1082130432, -1082130432, -1082130432, -1082130432);
	unk_0xBF3D28CA44F3BE2D(-1, "Menu_Navigate", &Global_20489, 1);
	func_34();
	return;
}

void func_34() // Position - 0x3544
{
	if (func_28())
		if (Global_20709 == 0)
			unk_0xBF3F90E670404C44(4);
		else
			unk_0xBF3F90E670404C44(3);

	return;
}

void func_35() // Position - 0x3567
{
	unk_0x0B0C9A0F9AAEB7F0(&Global_8372, 9);
	func_37(10, "CELL_16" /*Settings*/, 0, "appSettings", 24, 1, 1, 0, 0);
	func_113(Global_20481, "SET_DATA_SLOT_EMPTY", 1f, -1082130432, -1082130432, -1082130432, -1082130432);

	for (iLocal_70 = 0; iLocal_70 < 9; iLocal_70 = iLocal_70 + 1)
	{
		unk_0x88F483FBD433696A(Global_20481, "SET_DATA_SLOT");
		unk_0x330108B080A2132F(1);
		unk_0x330108B080A2132F(iLocal_70);
		unk_0x330108B080A2132F(25);
		unk_0x330108B080A2132F(0);
		func_36(&Global_8377[10 /*15*/]);
		unk_0x330108B080A2132F(225);
		unk_0xE6B753D52F4CA222();
	}

	Global_8903[0] = 10;
	unk_0x88F483FBD433696A(Global_20481, "SET_DATA_SLOT");
	unk_0x330108B080A2132F(1);
	unk_0x330108B080A2132F(0);
	unk_0x330108B080A2132F(Global_8377[10 /*15*/].f_10);
	unk_0x330108B080A2132F(0);
	func_36(&Global_8377[10 /*15*/]);
	unk_0x330108B080A2132F(255);
	unk_0xE6B753D52F4CA222();
	func_19();
	func_113(Global_20481, "DISPLAY_VIEW", 4f, SYSTEM::TO_FLOAT(0), -1082130432, -1082130432, -1082130432);
	func_113(Global_20481, "DISPLAY_VIEW", 1f, SYSTEM::TO_FLOAT(0), -1082130432, -1082130432, -1082130432);
	func_80();
	return;
}

void func_36(char* sParam0) // Position - 0x3681
{
	unk_0x882AEFD55B8D51FB(sParam0);
	unk_0xCFAD3D478C87321A();
	return;
}

void func_37(int iParam0, char* sParam1, int iParam2, char* sParam3, int iParam4, int iParam5, int iParam6, int iParam7, int iParam8) // Position - 0x3693
{
	TEXT_LABEL_ASSIGN_STRING(&Global_8377[iParam0 /*15*/], sParam1, 16);
	Global_8377[iParam0 /*15*/].f_4 = iParam2;
	TEXT_LABEL_ASSIGN_STRING(&(Global_8377[iParam0 /*15*/].f_5), sParam3, 16);
	Global_8377[iParam0 /*15*/].f_9 = unk_0x70E57E9927B6BA58(sParam3);
	Global_8377[iParam0 /*15*/].f_10 = iParam4;
	Global_8377[iParam0 /*15*/].f_11 = iParam5;
	Global_8377[iParam0 /*15*/].f_12 = iParam6;
	Global_8377[iParam0 /*15*/].f_13 = iParam7;
	Global_8377[iParam0 /*15*/].f_14 = iParam8;

	if (Global_8377[iParam0 /*15*/].f_12 == 0)
		Global_8377[iParam0 /*15*/].f_12 = 0;

	if (Global_8377[iParam0 /*15*/].f_13 == 0)
		Global_8377[iParam0 /*15*/].f_13 = 0;

	if (Global_8377[iParam0 /*15*/].f_14 == 0)
		Global_8377[iParam0 /*15*/].f_14 = 0;

	return;
}

void func_38() // Position - 0x3749
{
	if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
	{
		unk_0xBF3D28CA44F3BE2D(-1, "Menu_Accept", &Global_20489, 1);
		func_39();
	}

	return;
}

void func_39() // Position - 0x376F
{
	if (func_28())
		unk_0xBF3F90E670404C44(5);

	return;
}

BOOL func_40() // Position - 0x3783
{
	return false;
}

void func_41() // Position - 0x378C
{
	if (func_62(2, Global_20472, 0))
	{
		func_38();
		unk_0x8744D2E3FC95740E(&Global_8372, 9);
		func_113(Global_20481, "SET_DATA_SLOT_EMPTY", 1f, -1082130432, -1082130432, -1082130432, -1082130432);
		func_83(1);
		func_113(Global_20481, "DISPLAY_VIEW", 1f, SYSTEM::TO_FLOAT(Global_20504), -1082130432, -1082130432, -1082130432);
		func_80();
	}

	return;
}

void func_42() // Position - 0x37F6
{
	if (Global_20479)
		if (SYSTEM::TIMERA() > 50)
			Global_20479 = false;

	if (unk_0xAE231F549813BBDF(2))
	{
		if (unk_0x875A214D5EBCA509(2, 180))
		{
			switch (Global_20480)
			{
				case 6:
					if (Global_8939[7])
					{
						func_33();
						Global_20480 = 7;
					}
					break;
			
				case 7:
					if (Global_8939[6])
					{
						func_29();
						Global_20480 = 6;
					}
					break;
			}
		}
	
		if (unk_0x875A214D5EBCA509(2, 181))
		{
			switch (Global_20480)
			{
				case 6:
					if (Global_8939[7])
					{
						func_33();
						Global_20480 = 7;
					}
					break;
			
				case 7:
					if (Global_8939[6])
					{
						func_29();
						Global_20480 = 6;
					}
					break;
			}
		}
	}

	if (Global_20479 == false)
	{
		if (func_62(2, Global_20475, 0))
		{
			Global_20709 = 0;
		
			switch (Global_20480)
			{
				case 2:
					Global_20480 = 0;
					break;
			
				case 5:
					Global_20480 = 3;
					break;
			
				case 7:
					if (Global_8939[Global_20480 + 1] == true)
					{
						Global_20480 = 8;
					}
					else
					{
						Global_20480 = 6;
						Global_20709 = 1;
					}
					break;
			
				case 8:
					Global_20480 = 6;
					break;
			
				default:
					Global_20480 = Global_20480 + 1;
					break;
			}
		
			if (Global_20709 == 1)
				func_29();
			else
				func_33();
		
			Global_20479 = true;
			SYSTEM::SETTIMERA(0);
		}
	
		if (func_62(2, Global_20474, 0))
		{
			Global_20709 = 0;
		
			switch (Global_20480)
			{
				case 0:
					Global_20480 = 2;
					break;
			
				case 3:
					Global_20480 = 5;
					break;
			
				case 6:
					if (Global_8939[8] == true)
					{
						Global_20480 = 8;
					}
					else
					{
						Global_20480 = 7;
						Global_20709 = 1;
					}
					break;
			
				default:
					Global_20480 = Global_20480 - 1;
					break;
			}
		
			if (Global_20709 == 1)
				func_33();
			else
				func_29();
		
			Global_20479 = true;
			SYSTEM::SETTIMERA(0);
		}
	}

	return;
}

void func_43() // Position - 0x39B4
{
	if (Global_20479)
		if (SYSTEM::TIMERA() > 50)
			Global_20479 = false;

	if (unk_0xAE231F549813BBDF(2))
	{
		if (unk_0x875A214D5EBCA509(2, 180))
		{
			switch (Global_20480)
			{
				case 0:
					if (Global_8939[1])
					{
						func_33();
						Global_20480 = 1;
					}
					break;
			
				case 1:
					if (Global_8939[2])
					{
						func_33();
						Global_20480 = 2;
					}
					break;
			
				case 2:
					if (Global_8939[3])
					{
						func_33();
						func_31();
						Global_20480 = 3;
					}
					else
					{
						func_29();
						func_31();
						Global_20480 = 4;
					}
					break;
			
				case 3:
					if (Global_8939[4])
					{
						func_33();
						Global_20480 = 4;
					}
					break;
			
				case 4:
					if (Global_8939[5])
					{
						func_33();
						Global_20480 = 5;
					}
					break;
			
				case 5:
					if (Global_8939[6])
					{
						func_33();
						func_31();
						Global_20480 = 6;
					}
					break;
			
				case 6:
					if (Global_8939[7])
					{
						func_33();
						Global_20480 = 7;
					}
					break;
			
				case 7:
					if (Global_8939[8])
					{
						func_33();
						Global_20480 = 8;
					}
					else
					{
						func_29();
						func_31();
						Global_20480 = 0;
					}
					break;
			
				case 8:
					if (Global_8939[0])
					{
						func_33();
						func_31();
						Global_20480 = 0;
					}
					break;
			}
		}
	
		if (unk_0x875A214D5EBCA509(2, 181))
		{
			switch (Global_20480)
			{
				case 0:
					if (Global_8939[8])
					{
						func_29();
						func_26();
						Global_20480 = 8;
					}
					else
					{
						func_33();
						func_26();
						Global_20480 = 7;
					}
					break;
			
				case 1:
					if (Global_8939[0])
					{
						func_29();
						Global_20480 = 0;
					}
					break;
			
				case 2:
					if (Global_8939[1])
					{
						func_29();
						Global_20480 = 1;
					}
					break;
			
				case 3:
					if (Global_8939[2])
					{
						func_29();
						func_26();
						Global_20480 = 2;
					}
					break;
			
				case 4:
					if (Global_8939[3])
					{
						func_29();
						Global_20480 = 3;
					}
					else
					{
						func_26();
						func_33();
						Global_20709 = 1;
						Global_20480 = 2;
					}
					break;
			
				case 5:
					if (Global_8939[4])
					{
						func_29();
						Global_20480 = 4;
					}
					break;
			
				case 6:
					if (Global_8939[5])
					{
						func_26();
						func_29();
						Global_20480 = 5;
					}
					break;
			
				case 7:
					if (Global_8939[6])
					{
						func_29();
						Global_20480 = 6;
					}
					break;
			
				case 8:
					if (Global_8939[7])
					{
						func_29();
						Global_20480 = 7;
					}
					break;
			}
		}
	}

	if (Global_20479 == false)
	{
		if (func_62(2, Global_20475, 0))
		{
			Global_20709 = 0;
		
			switch (Global_20480)
			{
				case 2:
					Global_20480 = 0;
					break;
			
				case 5:
					Global_20480 = 3;
					break;
			
				case 7:
					if (Global_8939[Global_20480 + 1] == true)
					{
						Global_20480 = 8;
					}
					else
					{
						Global_20480 = 6;
						Global_20709 = 1;
					}
					break;
			
				case 8:
					Global_20480 = 6;
					break;
			
				default:
					Global_20480 = Global_20480 + 1;
					break;
			}
		
			if (Global_20709 == 1)
				func_29();
			else
				func_33();
		
			Global_20479 = true;
			SYSTEM::SETTIMERA(0);
		}
	
		if (func_62(2, Global_20474, 0))
		{
			Global_20709 = 0;
		
			switch (Global_20480)
			{
				case 0:
					Global_20480 = 2;
					break;
			
				case 3:
					Global_20480 = 5;
					break;
			
				case 6:
					if (Global_8939[8] == true)
					{
						Global_20480 = 8;
					}
					else
					{
						Global_20480 = 7;
						Global_20709 = 1;
					}
					break;
			
				default:
					Global_20480 = Global_20480 - 1;
					break;
			}
		
			if (Global_20709 == 1)
				func_33();
			else
				func_29();
		
			Global_20479 = true;
			SYSTEM::SETTIMERA(0);
		}
	
		if (func_62(2, Global_20476, 0))
		{
			Global_20709 = 0;
		
			switch (Global_20480)
			{
				case 0:
					if (Global_8939[6])
						Global_20480 = 6;
					break;
			
				case 1:
					if (Global_8939[7])
						Global_20480 = 7;
					break;
			
				case 2:
					if (Global_8939[8])
					{
						Global_20480 = 8;
					}
					else
					{
						Global_20709 = 1;
						Global_20480 = 5;
					}
					break;
			
				case 3:
					if (Global_8939[0])
						Global_20480 = 0;
					break;
			
				case 4:
					if (Global_8939[1])
						Global_20480 = 1;
					break;
			
				case 5:
					if (Global_8939[2])
						Global_20480 = 2;
					break;
			
				case 6:
					if (Global_8939[3])
						Global_20480 = 3;
					break;
			
				case 7:
					if (Global_8939[4])
						Global_20480 = 4;
					break;
			
				case 8:
					if (Global_8939[5])
						Global_20480 = 5;
					break;
			}
		
			if (Global_20709 == 1)
				func_31();
			else
				func_26();
		
			Global_20479 = true;
			SYSTEM::SETTIMERA(0);
		}
	
		if (func_62(2, Global_20477, 0))
		{
			Global_20709 = 0;
		
			switch (Global_20480)
			{
				case 0:
					if (Global_8939[3])
						Global_20480 = 3;
					break;
			
				case 1:
					if (Global_8939[4])
						Global_20480 = 4;
					break;
			
				case 2:
					if (Global_8939[5])
						Global_20480 = 5;
					break;
			
				case 3:
					if (Global_8939[6])
						Global_20480 = 6;
					break;
			
				case 4:
					if (Global_8939[7])
						Global_20480 = 7;
					break;
			
				case 5:
					if (Global_8939[8])
					{
						Global_20480 = 8;
					}
					else
					{
						Global_20709 = 1;
						Global_20480 = 2;
					}
					break;
			
				case 6:
					if (Global_8939[0])
						Global_20480 = 0;
					break;
			
				case 7:
					if (Global_8939[1])
						Global_20480 = 1;
					break;
			
				case 8:
					if (Global_8939[2])
						Global_20480 = 2;
					break;
			}
		
			if (Global_20709 == 1)
				func_26();
			else
				func_31();
		
			Global_20479 = true;
			SYSTEM::SETTIMERA(0);
		}
	}

	return;
}

void func_44() // Position - 0x3F46
{
	int num;
	var unk;
	char* str;

	if (unk_0xA6F779AA284EB20E(uLocal_72))
	{
		bLocal_73 = false;
		iLocal_71 = unk_0x5DEF122A58D4F685(uLocal_72);
	
		if (Global_20705 == false)
			Global_20504 = iLocal_71;
		else
			Global_20505 = iLocal_71;
	
		if (iLocal_71 < 0)
			iLocal_71 = 0;
	
		Global_20503 = Global_8903[iLocal_71];
	
		if (IS_BIT_SET(Global_8371, 10))
		{
			Global_20503 = 2;
			unk_0x8744D2E3FC95740E(&Global_8371, 10);
		}
	
		if (iLocal_121 == 1)
		{
			Global_20504 = 1;
			Global_20503 = 0;
			Global_2804775 = 0;
			iLocal_121 = 0;
		}
	
		num = 0;
	
		if (Global_78689)
			Global_20503 == 24 && func_40();
	
		if (unk_0xF40767E41852FB72(Global_20498))
			num = 1;
	
		if (Global_20503 == 3)
		{
			if (IS_BIT_SET(Global_8371, 3))
			{
				num = 1;
			}
			else if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
			{
				if (unk_0x1BAABBB7277FCD90(unk_0x4A8C381C258A124D()) || unk_0xF6D8C7059C3C2495() || unk_0xB96BD3A32937D44E(unk_0x259BE71D8A81D4FA()) || unk_0x49CB27F641DC1328(unk_0x4A8C381C258A124D()) || unk_0xC3174723CF144560(unk_0x4A8C381C258A124D()) || unk_0xFFAC548682B3D56E(unk_0x4A8C381C258A124D()) || unk_0xD9F5E1FEFD1329E4(unk_0x31945A289F1359A1(unk_0x4A8C381C258A124D()), 0) || unk_0xDC7D2B438B687A13())
					num = 1;
			
				if (unk_0x7F420695E3F776FB(unk_0x4A8C381C258A124D(), 0))
				{
					if (unk_0x27E68848F0E5D7D9(unk_0x4A8C381C258A124D()) == 0)
						num = 1;
				
					unk = unk_0x6EF03BE64E058E2F(unk_0x4A8C381C258A124D(), 0);
				
					if (unk_0x032CEF99C0D59B73(unk) < 0f)
						num = 1;
				}
			
				if (unk_0x4F035D45FC2856F8(unk_0x259BE71D8A81D4FA()))
					num = 1;
			
				if (Global_78689)
				{
					if (unk_0x6D05C5731A838CB3(0, 25) || unk_0x6D05C5731A838CB3(0, 68))
					{
						unk_0x23B29877D0BE9547(unk_0x4A8C381C258A124D(), &iLocal_112, 1);
					
						if (iLocal_112 != joaat("WEAPON_UNARMED"))
							num = 1;
					}
				}
			}
		}
	
		if (Global_20503 == 2)
			if (IS_BIT_SET(Global_8371, 6))
				num = 1;
	
		if (Global_20503 == 23 && unk_0x1B79E937E91F40C3(&Global_8377[Global_20503 /*15*/], "CELL_BENWEB" /*Benny's*/))
			if (IS_BIT_SET(Global_8371, 6))
				num = 1;
	
		if (Global_78689)
		{
			if (Global_20503 == 15 || Global_20503 == 5)
			{
			}
			else
			{
				if (Global_20503 == 23)
				{
					IS_BIT_SET(Global_4542302, 4) == false && Global_1836104 == false && IS_BIT_SET(Global_4542302, 20) == false && IS_BIT_SET(Global_4542302, 22) == false;
				
					if (IS_BIT_SET(Global_4542302, 20) == true && IS_BIT_SET(Global_4542302, 4) == false && Global_1836104 == false && IS_BIT_SET(Global_4542302, 22) == false && IS_BIT_SET(Global_4542302, 26) == false)
					{
						num = 1;
						unk_0x0B0C9A0F9AAEB7F0(&Global_4542302, 21);
					}
				}
			
				if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
					if (unk_0x49F8918E2DC5FFBC(unk_0x4A8C381C258A124D(), 78, 1) || unk_0x1C751EF63BF4D501(unk_0x259BE71D8A81D4FA()))
						if (Global_20503 == 3)
							num = 1;
			
				if (Global_75815 && Global_20503 == 2)
				{
					num = 1;
					str = func_47();
				
					if (!func_46(str))
						func_78(str, -1);
				}
			
				if (func_122())
				{
					if (Global_20503 == 10)
					{
						if (!func_46("FIX_MPCHAR_BLCK" /*This app is currently unavailable. Please try again later.*/))
							func_78("FIX_MPCHAR_BLCK" /*This app is currently unavailable. Please try again later.*/, -1);
					
						num = 1;
					}
				}
			
				if (num == 0)
				{
					func_81(Global_20481, "SET_SOFT_KEYS", 1f, 0f, 1f, -1f, -1f, 0, 0, 0, 0, 0);
					unk_0x8744D2E3FC95740E(&Global_8370, 17);
					Global_20500.f_1 = 7;
				
					if (IS_BIT_SET(Global_8370, 23))
					{
					}
					else
					{
						func_38();
						unk_0x97A5024CE91641F1(&(Global_8377[Global_20503 /*15*/].f_5));
						unk_0x0B0C9A0F9AAEB7F0(&Global_8370, 23);
					}
				}
				else
				{
					Global_20503 == 2 || Global_20503 == 3;
					func_45();
				}
			}
		}
		else
		{
			switch (Global_20503)
			{
				case 3:
					if (Global_112863 == 1)
						num = 1;
					break;
			
				case 8:
					num = 1;
					break;
			
				case 15:
					num = 1;
					break;
			
				case 16:
					num = 1;
					unk_0x0B0C9A0F9AAEB7F0(&Global_8370, 26);
					break;
			
				case 5:
					num = 1;
					break;
			
				case 20:
					num = 1;
				
					if (Global_20707 == 0)
					{
						if (Global_8374 == 0 && iLocal_74 == 0 && Global_43377 == 15)
						{
							iLocal_74 = 1;
							func_38();
							!Global_113810.f_14054.f_85;
						}
					}
					else
					{
						func_45();
					}
					break;
			
				default:
					break;
			}
		
			if (unk_0xAF078BC665CCACB9())
				if (Global_20503 != 0)
					num = 1;
		
			if (func_82(14))
				if (Global_20503 != 3 && Global_20503 != 14)
					num = 1;
		
			if (num == 0)
			{
				func_38();
				func_81(Global_20481, "SET_SOFT_KEYS", 1f, 0f, 1f, -1f, -1f, 0, 0, 0, 0, 0);
				unk_0x8744D2E3FC95740E(&Global_8370, 17);
				Global_20500.f_1 = 7;
			
				if (IS_BIT_SET(Global_8370, 23))
				{
				}
				else
				{
					unk_0x97A5024CE91641F1(&(Global_8377[Global_20503 /*15*/].f_5));
					unk_0x0B0C9A0F9AAEB7F0(&Global_8370, 23);
				}
			}
			else if (Global_20503 != 20)
			{
				if (Global_113810.f_14054.f_86 == 0)
				{
					if (unk_0x486FF5D06E9659F1(joaat("paparazzo3b")) > 0)
					{
						if (Global_20503 == 3)
						{
							if (unk_0x75EAB09F5E974116(unk_0x259BE71D8A81D4FA()))
							{
								if (unk_0xD6179D448A06A77F(unk_0x4A8C381C258A124D(), 0))
								{
									func_78("CELL_38" /*You cannot use the Snapmatic application while in cover.*/, -1);
									Global_113810.f_14054.f_86 = 1;
								}
							}
						}
					}
				}
			
				func_45();
			}
		}
	}

	return;
}

void func_45() // Position - 0x4437
{
	if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
		unk_0xBF3D28CA44F3BE2D(-1, "Menu_Back", &Global_20489, 1);

	return;
}

BOOL func_46(char* sParam0) // Position - 0x4459
{
	unk_0x39DCBE5519BD783A(sParam0);
	return unk_0x7EBCD400E7DE179C(0);
}

char* func_47() // Position - 0x446C
{
	return "BLOCK_APP_WEB" /*The Internet app is currently unavailable. Please try again later.*/;
}

void func_48() // Position - 0x4478
{
	if (Global_20466 == 0)
	{
		if (func_15(Global_20453[Global_20445 /*3*/], Global_20446[Global_20445 /*3*/], Global_20460, Global_20460, 350f, false) >= 1f)
		{
			iLocal_16 = 0;
			unk_0x8744D2E3FC95740E(&Global_8371, 24);
			unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 26);
		}
	}

	return;
}

void func_49() // Position - 0x44C9
{
	if (Global_20466 == 0)
	{
		if (func_15(Global_20446[Global_20445 /*3*/], Global_20453[Global_20445 /*3*/], Global_20460, Global_20460, 350f, true) >= 1f)
		{
			iLocal_16 = 0;
			unk_0x8744D2E3FC95740E(&Global_8371, 25);
		
			if (Global_21858)
				unk_0x8744D2E3FC95740E(&Global_8371, 27);
		}
	}

	return;
}

void func_50() // Position - 0x4520
{
	if (Global_20466 == 0 && Global_20440 == 0)
	{
		if (func_15(Global_20453[Global_20445 /*3*/], Global_20446[Global_20445 /*3*/], Global_20460, Global_20460, 350f, false) >= 1f)
		{
			iLocal_16 = 0;
			Global_4542581 = 0;
		}
	}

	return;
}

void func_51() // Position - 0x456F
{
	float num;
	float num2;

	if (IS_BIT_SET(Global_8372, 2))
	{
		unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 8);
		unk_0x0B0C9A0F9AAEB7F0(&Global_8370, 14);
		Global_20440 = 0;
		iLocal_16 = 0;
		unk_0xA0382000AA987093(Global_20460, 0);
	}
	else if (unk_0x486FF5D06E9659F1(joaat("appemail")) < 1)
	{
		num = func_15(Global_20446[Global_20445 /*3*/], Global_20453[Global_20445 /*3*/], Global_20463, Global_20460, 450f, true);
	
		if (num >= 1f)
		{
			unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 8);
			unk_0x0B0C9A0F9AAEB7F0(&Global_8370, 14);
			Global_20440 = 0;
			iLocal_16 = 0;
			unk_0x8744D2E3FC95740E(&Global_8372, 2);
			iLocal_81 = 0;
		}
		else if (num >= 0.75f)
		{
			unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 8);
		}
	}
	else
	{
		if (iLocal_81 == 0)
		{
			fLocal_82 = { Global_20453[Global_20445 /*3*/] };
			fLocal_82 = fLocal_82 - 10f;
			fLocal_82.f_1 = fLocal_82.f_1 + 20f;
			iLocal_81 = 1;
		}
	
		num2 = func_15(Global_20446[Global_20445 /*3*/], fLocal_82, -90f, 0f, 90f, -90f, 0f, 90f, 450f, true);
	
		if (num2 >= 1f)
		{
			unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 8);
			unk_0x0B0C9A0F9AAEB7F0(&Global_8370, 14);
			Global_20440 = 0;
			iLocal_16 = 0;
			unk_0x8744D2E3FC95740E(&Global_8372, 2);
			iLocal_81 = 0;
		}
		else if (num2 >= 0.75f)
		{
			unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 8);
		}
	}

	return;
}

void func_52() // Position - 0x46C2
{
	if (unk_0x76CD105BCAC6EB9F())
	{
		iLocal_106 = unk_0x775142054EC39277(unk_0x7E3F74F641EE6B27(), uLocal_105);
	}
	else
	{
		iLocal_101 = unk_0x1DD05E817C89C737();
		iLocal_106 = iLocal_101 - uLocal_100;
	}

	if (iLocal_106 > 4000)
	{
		iLocal_88 = func_104();
		unk_0xE87F28FD4128D063();
	
		if (unk_0x76CD105BCAC6EB9F())
			uLocal_105 = unk_0x7E3F74F641EE6B27();
		else
			uLocal_100 = unk_0x1DD05E817C89C737();
	}

	return;
}

void func_53() // Position - 0x4718
{
	if (Global_20499 == 1)
	{
		func_121();
	
		if (Global_20478 == 0)
		{
			if (func_62(2, Global_20470, 0))
			{
				if (IS_BIT_SET(Global_8371, 8))
				{
					if (Global_21892 == false)
					{
						func_45();
						Global_20478 = 1;
						Global_20499 = 3;
						Global_21894 = 1;
					
						if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
							unk_0x92187CF73A8D1B96(unk_0x4A8C381C258A124D());
					}
				}
			}
		}
	
		if (Global_20478 == 0)
		{
			if (func_62(2, Global_20469, 0))
			{
				if (IS_BIT_SET(Global_8371, 8))
				{
					func_38();
					Global_20478 = 1;
					Global_20499 = 2;
				
					if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
						unk_0x92187CF73A8D1B96(unk_0x4A8C381C258A124D());
				
					func_59();
					func_58();
				}
			}
		}
	}
	else
	{
		if (iLocal_98 == 0)
		{
			if (IS_BIT_SET(Global_8370, 27))
			{
				iLocal_98 = 1;
				SYSTEM::SETTIMERB(0);
			}
		}
		else
		{
			if (Global_20500.f_1 > 3)
				if (SYSTEM::TIMERB() > 1500)
					if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
						if (!unk_0x129DE7ED6456C4CA(unk_0x4A8C381C258A124D()))
							if (Global_20443)
								unk_0x1BC60757AC20ACCF("Dial_and_Remote_Ring", unk_0x4A8C381C258A124D(), 1);
							else if (!IS_BIT_SET(Global_8372, 5))
								unk_0x1BC60757AC20ACCF("Remote_Ring", unk_0x4A8C381C258A124D(), 1);
		
			if (!IS_BIT_SET(Global_8370, 27))
			{
				iLocal_98 = 0;
			
				if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
					unk_0x92187CF73A8D1B96(unk_0x4A8C381C258A124D());
			}
		}
	
		if (Global_20499 == 0)
		{
			if (Global_20478 == 0)
			{
				if (func_62(2, Global_20468, 0) || Global_2804775 == 1)
				{
					if (unk_0x75EAB09F5E974116(unk_0x259BE71D8A81D4FA()))
					{
						if (unk_0xE916D57851F785AB(unk_0x259BE71D8A81D4FA()))
						{
							if (IS_BIT_SET(Global_8371, 8))
							{
								switch (Global_20500.f_1)
								{
									case 3:
										break;
								
									case 4:
										break;
								
									case 6:
										if (Global_20704 == false)
										{
											if (Global_20705 == true)
											{
												func_45();
												Global_20705 = false;
												func_113(Global_20481, "DISPLAY_VIEW", 4f, -1082130432, -1082130432, -1082130432, -1082130432);
												func_85();
												func_84();
												func_83(1);
												func_113(Global_20481, "DISPLAY_VIEW", 1f, SYSTEM::TO_FLOAT(Global_20504), -1082130432, -1082130432, -1082130432);
											}
											else
											{
												if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
													unk_0xBF3D28CA44F3BE2D(-1, "Put_Away", &Global_20489, 1);
											
												iLocal_16 = 0;
												Global_20500.f_1 = 3;
											}
										}
										break;
								
									case 5:
										Global_20500.f_1 = 3;
										break;
								
									case 7:
										IS_BIT_SET(Global_8370, 23) == true;
									
										if (Global_8978 - Global_8977 > Global_8979 && IS_BIT_SET(Global_8370, 23) == false)
										{
											if (IS_BIT_SET(Global_8371, 0))
											{
											}
											else
											{
												func_45();
												Global_8976 = 1;
												Global_20500.f_1 = 6;
											
												if (Global_78689)
													func_113(Global_20481, "SET_THEME", SYSTEM::TO_FLOAT(Global_4542578), -1082130432, -1082130432, -1082130432, -1082130432);
											
												Global_8373 = 99;
											
												if (Global_2804775 == 0)
													func_54();
											}
										}
										break;
								
									case 8:
										break;
								
									case 10:
										if (Global_7685 == 132)
										{
											if (Global_2801145.f_1 || IS_BIT_SET(Global_8371, 20))
											{
												func_45();
												func_76();
											}
										}
										else
										{
											func_45();
											func_76();
											Global_21894 = 1;
										}
										break;
								
									case 9:
										if (Global_21845 == 0)
											Global_20500.f_1 = 3;
										break;
								
									default:
										break;
								}
							
								if (Global_2804775 == 1)
								{
									iLocal_121 = 1;
									Global_2804775 = 0;
								}
							}
						}
					}
				}
			}
		}
	}

	return;
}

void func_54() // Position - 0x4A45
{
	var unk;

	if (Global_20483 == 1)
		return;

	if (Global_20500.f_1 < 4)
		return;

	while (!unk_0xA0C7B98BCF1EEF9E(Global_20481))
	{
		if (Global_78689)
			return;
	
		SYSTEM::WAIT(0);
	}

	switch (Global_20500.f_1)
	{
		case 6:
			func_113(Global_20481, "SET_DATA_SLOT_EMPTY", 1f, -1082130432, -1082130432, -1082130432, -1082130432);
			func_83(Global_8975);
		
			if (Global_8975 == 1)
			{
				func_113(Global_20481, "DISPLAY_VIEW", 1f, SYSTEM::TO_FLOAT(Global_20504), -1082130432, -1082130432, -1082130432);
				Global_20480 = Global_20504;
			}
			else
			{
				func_113(Global_20481, "DISPLAY_VIEW", 1f, SYSTEM::TO_FLOAT(Global_20505), -1082130432, -1082130432, -1082130432);
				Global_20480 = Global_20505;
			}
		
			if (Global_20488)
			{
				func_81(Global_20481, "SET_SOFT_KEYS", 2f, 1f, 2f, -1f, -1f, "CELL_205" /*SELECT*/, 0, 0, 0, 0);
				func_81(Global_20481, "SET_SOFT_KEYS", 3f, 1f, 4f, -1f, -1f, "CELL_206" /*BACK*/, 0, 0, 0, 0);
			}
			else
			{
				func_81(Global_20481, "SET_SOFT_KEYS", 2f, 1f, 2f, -1f, -1f, 0, 0, 0, 0, 0);
				func_81(Global_20481, "SET_SOFT_KEYS", 3f, 1f, 4f, -1f, -1f, 0, 0, 0, 0, 0);
			}
		
			if (Global_20706 == false)
			{
				func_81(Global_20481, "SET_SOFT_KEYS", 1f, 0f, 1f, -1f, -1f, 0, 0, 0, 0, 0);
				unk_0x8744D2E3FC95740E(&Global_8370, 17);
			}
			else if (Global_78689)
			{
				func_81(Global_20481, "SET_SOFT_KEYS", 1f, 0f, 1f, -1f, -1f, 0, 0, 0, 0, 0);
				unk_0x8744D2E3FC95740E(&Global_8370, 17);
			}
			else
			{
				if (Global_20705 == true)
					if (Global_20488)
						func_81(Global_20481, "SET_SOFT_KEYS", 1f, 1f, 20f, -1f, -1f, "CELL_225" /*NEXT*/, 0, 0, 0, 0);
					else
						func_81(Global_20481, "SET_SOFT_KEYS", 1f, 1f, 20f, -1f, -1f, 0, 0, 0, 0, 0);
				else if (Global_20488)
					func_81(Global_20481, "SET_SOFT_KEYS", 1f, 1f, 3f, -1f, -1f, "CELL_225" /*NEXT*/, 0, 0, 0, 0);
				else
					func_81(Global_20481, "SET_SOFT_KEYS", 1f, 1f, 3f, -1f, -1f, 0, 0, 0, 0, 0);
			
				unk_0x0B0C9A0F9AAEB7F0(&Global_8370, 17);
			}
		
			if (Global_78689)
			{
				func_80();
				unk_0x8744D2E3FC95740E(&Global_8372, 9);
				func_113(Global_20481, "DISPLAY_VIEW", 1f, SYSTEM::TO_FLOAT(Global_20504), -1082130432, -1082130432, -1082130432);
			}
			break;
	
		case 7:
			break;
	
		case 10:
			func_113(Global_20481, "DISPLAY_VIEW", 4f, -1082130432, -1082130432, -1082130432, -1082130432);
			func_81(Global_20481, "SET_SOFT_KEYS", 2f, 0f, 1f, -1f, -1f, 0, 0, 0, 0, 0);
			func_81(Global_20481, "SET_SOFT_KEYS", 1f, 0f, 1f, -1f, -1f, 0, 0, 0, 0, 0);
			unk_0x8744D2E3FC95740E(&Global_8370, 17);
		
			if (IS_BIT_SET(Global_8370, 20))
			{
				func_81(Global_20481, "SET_SOFT_KEYS", 3f, 0f, 1f, -1f, -1f, 0, 0, 0, 0, 0);
			}
			else if (Global_20488)
			{
				func_81(Global_20481, "SET_SOFT_KEYS", 3f, 1f, 6f, -1f, -1f, "CELL_202" /*END CALL*/, 0, 0, 0, 0);
			
				if (Global_20443)
					func_81(Global_20481, "SET_SOFT_KEYS", 3f, 1f, 6f, -1f, -1f, 0, 0, 0, 0, 0);
			}
			else
			{
				func_81(Global_20481, "SET_SOFT_KEYS", 3f, 1f, 6f, -1f, -1f, 0, 0, 0, 0, 0);
			}
			break;
	
		case 9:
			if (Global_20499 == 1)
			{
				func_106();
				func_113(Global_20481, "SET_THEME", SYSTEM::TO_FLOAT(Global_113810.f_14054[Global_20500 /*20*/].f_6), -1082130432, -1082130432, -1082130432, -1082130432);
			
				if (Global_21858)
				{
					unk_0x88F483FBD433696A(Global_20481, "SET_DATA_SLOT");
					unk_0x330108B080A2132F(4);
					unk_0x330108B080A2132F(0);
					unk_0x330108B080A2132F(2);
					unk_0x882AEFD55B8D51FB("CELL_CONDFON" /*~a~*/);
					unk_0x60D332F23943B34F(&Global_21860);
					unk_0xCFAD3D478C87321A();
					func_36("CELL_300" /*CHAR_DEFAULT*/);
					func_36("CELL_217" /*INCOMING CALL*/);
					func_36("CELL_217" /*INCOMING CALL*/);
					unk_0xE6B753D52F4CA222();
				}
				else if (func_57(Global_7685, Global_20500) == 0)
				{
					func_81(Global_20481, "SET_DATA_SLOT", SYSTEM::TO_FLOAT(4), SYSTEM::TO_FLOAT(0), SYSTEM::TO_FLOAT(2), -1f, -1f, &(Global_117[Global_7685 /*10*/].f_4), "CELL_300" /*CHAR_DEFAULT*/, "CELL_217" /*INCOMING CALL*/, "CELL_195" /*Unknown*/, 0);
				}
				else
				{
					func_81(Global_20481, "SET_DATA_SLOT", SYSTEM::TO_FLOAT(4), SYSTEM::TO_FLOAT(0), SYSTEM::TO_FLOAT(2), -1f, -1f, &(Global_2058[Global_7685 /*29*/].f_3), &(Global_2058[Global_7685 /*29*/].f_7), "CELL_217" /*INCOMING CALL*/, &(Global_2058[Global_7685 /*29*/].f_3), 0);
				}
			
				func_113(Global_20481, "DISPLAY_VIEW", 4f, -1082130432, -1082130432, -1082130432, -1082130432);
			}
			else if (Global_21845 == 4 || Global_21845 == 3)
			{
				func_113(Global_20481, "SET_THEME", SYSTEM::TO_FLOAT(Global_113810.f_14054[Global_20500 /*20*/].f_6), -1082130432, -1082130432, -1082130432, -1082130432);
				func_106();
			
				if (Global_21858)
				{
					unk_0x88F483FBD433696A(Global_20481, "SET_DATA_SLOT");
					unk_0x330108B080A2132F(4);
					unk_0x330108B080A2132F(0);
					unk_0x330108B080A2132F(2);
					unk_0x882AEFD55B8D51FB("CELL_CONDFON" /*~a~*/);
					unk_0x60D332F23943B34F(&Global_21860);
					unk_0xCFAD3D478C87321A();
					func_36("CELL_300" /*CHAR_DEFAULT*/);
					func_36("CELL_219" /*CONNECTED*/);
					func_36("CELL_219" /*CONNECTED*/);
					unk_0xE6B753D52F4CA222();
				}
				else
				{
					if (Global_22103)
						TEXT_LABEL_ASSIGN_STRING(&unk, "CELL_219" /*CONNECTED*/, 24);
					else
						TEXT_LABEL_ASSIGN_STRING(&unk, "CELL_211" /*DIALING...*/, 24);
				
					if (func_57(Global_7685, Global_20500) == 0)
					{
						func_113(Global_20481, "SET_DATA_SLOT_EMPTY", 4f, -1082130432, -1082130432, -1082130432, -1082130432);
						func_81(Global_20481, "SET_DATA_SLOT", SYSTEM::TO_FLOAT(4), SYSTEM::TO_FLOAT(0), SYSTEM::TO_FLOAT(3), -1f, -1f, &(Global_117[Global_7685 /*10*/].f_4), "CELL_300" /*CHAR_DEFAULT*/, &unk, "CELL_195" /*Unknown*/, 0);
					}
					else
					{
						func_113(Global_20481, "SET_DATA_SLOT_EMPTY", 4f, -1082130432, -1082130432, -1082130432, -1082130432);
						func_81(Global_20481, "SET_DATA_SLOT", SYSTEM::TO_FLOAT(4), SYSTEM::TO_FLOAT(0), SYSTEM::TO_FLOAT(3), -1f, -1f, &(Global_2058[Global_7685 /*29*/].f_3), &(Global_2058[Global_7685 /*29*/].f_7), &unk, &(Global_2058[Global_7685 /*29*/].f_3), 0);
					}
				}
			
				func_113(Global_20481, "DISPLAY_VIEW", 4f, -1082130432, -1082130432, -1082130432, -1082130432);
			}
		
			func_55();
			break;
	
		default:
			break;
	}

	return;
}

void func_55() // Position - 0x5018
{
	if (unk_0xA0C7B98BCF1EEF9E(Global_20481))
	{
		func_56();
	
		if (Global_20499 == 1)
		{
			if (Global_20488)
				func_81(Global_20481, "SET_SOFT_KEYS", 2f, 1f, 5f, -1f, -1f, "CELL_203" /*ANSWER*/, 0, 0, 0, 0);
			else
				func_81(Global_20481, "SET_SOFT_KEYS", 2f, 1f, 5f, -1f, -1f, 0, 0, 0, 0, 0);
		
			if (Global_21892)
				func_81(Global_20481, "SET_SOFT_KEYS", 3f, 0f, 6f, -1f, -1f, 0, 0, 0, 0, 0);
			else if (IS_BIT_SET(Global_8370, 20))
				func_81(Global_20481, "SET_SOFT_KEYS", 3f, 0f, 1f, -1f, -1f, 0, 0, 0, 0, 0);
			else if (Global_20488)
				func_81(Global_20481, "SET_SOFT_KEYS", 3f, 1f, 6f, -1f, -1f, "CELL_204" /*REJECT*/, 0, 0, 0, 0);
			else
				func_81(Global_20481, "SET_SOFT_KEYS", 3f, 1f, 6f, -1f, -1f, 0, 0, 0, 0, 0);
		
			func_81(Global_20481, "SET_SOFT_KEYS", 1f, 0f, 1f, -1f, -1f, 0, 0, 0, 0, 0);
			unk_0x8744D2E3FC95740E(&Global_8370, 17);
		}
		else
		{
			func_81(Global_20481, "SET_SOFT_KEYS", 2f, 0f, 1f, -1f, -1f, 0, 0, 0, 0, 0);
			func_81(Global_20481, "SET_SOFT_KEYS", 1f, 0f, 1f, -1f, -1f, 0, 0, 0, 0, 0);
			unk_0x8744D2E3FC95740E(&Global_8370, 17);
		
			if (IS_BIT_SET(Global_8370, 20))
			{
				func_81(Global_20481, "SET_SOFT_KEYS", 3f, 0f, 1f, -1f, -1f, 0, 0, 0, 0, 0);
			}
			else if (Global_20488)
			{
				func_81(Global_20481, "SET_SOFT_KEYS", 3f, 1f, 6f, -1f, -1f, "CELL_202" /*END CALL*/, 0, 0, 0, 0);
			
				if (Global_20443)
					func_81(Global_20481, "SET_SOFT_KEYS", 3f, 1f, 6f, -1f, -1f, 0, 0, 0, 0, 0);
			}
			else
			{
				func_81(Global_20481, "SET_SOFT_KEYS", 3f, 1f, 6f, -1f, -1f, 0, 0, 0, 0, 0);
			}
		}
	}

	return;
}

void func_56() // Position - 0x5199
{
	if (Global_78689)
	{
		func_81(Global_20481, "SET_SOFT_KEYS", 1f, 0f, 9f, -1f, -1f, 0, 0, 0, 0, 0);
		unk_0x8744D2E3FC95740E(&Global_8370, 17);
	}

	return;
}

int func_57(int iParam0, int iParam1) // Position - 0x51C9
{
	if (iParam1 < 0 || iParam1 > 4)
		return 0;

	return Global_2058[iParam0 /*29*/].f_24[iParam1];
}

void func_58() // Position - 0x51F3
{
	if (Global_21858)
	{
		if (Global_78689)
		{
			if (unk_0xA0C7B98BCF1EEF9E(Global_20481))
			{
				if (!unk_0x761778199FE1211C())
				{
					func_81(Global_20481, "SET_SOFT_KEYS", 1f, 1f, 8f, -1f, -1f, "CELL_201" /*CALL*/, 0, 0, 0, 0);
					unk_0x0B0C9A0F9AAEB7F0(&(Global_2794162.f_1832), 15);
				}
			}
		}
	}

	return;
}

void func_59() // Position - 0x5242
{
	if (unk_0xA0C7B98BCF1EEF9E(Global_20481))
	{
		if (!Global_21853)
		{
			func_81(Global_20481, "SET_SOFT_KEYS", 2f, 0f, 1f, -1f, -1f, 0, 0, 0, 0, 0);
			func_81(Global_20481, "SET_SOFT_KEYS", 1f, 0f, 1f, -1f, -1f, 0, 0, 0, 0, 0);
			unk_0x8744D2E3FC95740E(&Global_8370, 17);
		
			if (IS_BIT_SET(Global_8370, 20))
			{
				func_81(Global_20481, "SET_SOFT_KEYS", 3f, 0f, 1f, -1f, -1f, 0, 0, 0, 0, 0);
			}
			else if (Global_20488)
			{
				func_81(Global_20481, "SET_SOFT_KEYS", 3f, 1f, 6f, -1f, -1f, "CELL_202" /*END CALL*/, 0, 0, 0, 0);
			
				if (Global_20443)
					func_81(Global_20481, "SET_SOFT_KEYS", 3f, 1f, 6f, -1f, -1f, 0, 0, 0, 0, 0);
			}
			else
			{
				func_81(Global_20481, "SET_SOFT_KEYS", 3f, 1f, 6f, -1f, -1f, 0, 0, 0, 0, 0);
			}
		
			if (Global_21858)
			{
				unk_0x88F483FBD433696A(Global_20481, "SET_DATA_SLOT");
				unk_0x330108B080A2132F(4);
				unk_0x330108B080A2132F(0);
				unk_0x330108B080A2132F(2);
				unk_0x882AEFD55B8D51FB("CELL_CONDFON" /*~a~*/);
				unk_0x60D332F23943B34F(&Global_21860);
				unk_0xCFAD3D478C87321A();
				func_36("CELL_300" /*CHAR_DEFAULT*/);
				func_36("CELL_219" /*CONNECTED*/);
				func_36("CELL_219" /*CONNECTED*/);
				unk_0xE6B753D52F4CA222();
			}
			else if (func_57(Global_7685, Global_20500) == 0)
			{
				func_81(Global_20481, "SET_DATA_SLOT", SYSTEM::TO_FLOAT(4), SYSTEM::TO_FLOAT(0), SYSTEM::TO_FLOAT(3), -1f, -1f, &(Global_117[Global_7685 /*10*/].f_4), "CELL_300" /*CHAR_DEFAULT*/, "CELL_219" /*CONNECTED*/, "CELL_195" /*Unknown*/, 0);
			}
			else
			{
				func_81(Global_20481, "SET_DATA_SLOT", SYSTEM::TO_FLOAT(4), SYSTEM::TO_FLOAT(0), SYSTEM::TO_FLOAT(3), -1f, -1f, &(Global_2058[Global_7685 /*29*/].f_3), &(Global_2058[Global_7685 /*29*/].f_7), "CELL_219" /*CONNECTED*/, &(Global_2058[Global_7685 /*29*/].f_3), 0);
			}
		}
	
		func_113(Global_20481, "DISPLAY_VIEW", 4f, -1082130432, -1082130432, -1082130432, -1082130432);
	}

	return;
}

void func_60() // Position - 0x53EF
{
	Global_20711 = 0;
	func_61();
	return;
}

void func_61() // Position - 0x53FF
{
	if (unk_0xE87F28FD4128D063() || Global_20499 == 1)
	{
		unk_0xC78B293A5F4EACF9();
		Global_22856 = 0;
		unk_0x0F15249D24BC5ADA(0);
		Global_21845 = 6;
		Global_20500.f_1 = Global_20502;
		return;
	}

	return;
}

BOOL func_62(int iParam0, var uParam1, int iParam2) // Position - 0x5436
{
	if (unk_0x875A214D5EBCA509(iParam0, uParam1) || iParam2 == 1 && unk_0xDEE3EFEA31A1F555(iParam0, uParam1))
	{
		if (unk_0x761778199FE1211C())
			if (unk_0x66DA7155B68E7638() == 0 || unk_0xA43CD45F18522E3F() && unk_0xAE231F549813BBDF(2))
				return false;
	
		if (unk_0x4D9174D8796EA622() || unk_0xB11671B812399BA2())
			return false;
		else
			return true;
	}

	return false;
}

BOOL func_63() // Position - 0x54A8
{
	if (unk_0x486FF5D06E9659F1(joaat("apptrackify")) > 0)
		return true;

	return false;
}

BOOL func_64(BOOL bParam0) // Position - 0x54C5
{
	if (!bParam0 && unk_0x486FF5D06E9659F1(joaat("benchmark")) > 0)
		return true;

	return IS_BIT_SET(Global_78938, 0);
}

void func_65(int iParam0) // Position - 0x54ED
{
	if (func_67())
		return;

	if (Global_20704)
		if (func_7())
			func_5(true, true);
		else
			func_5(false, false);

	if (Global_20500.f_1 == 10 || Global_20500.f_1 == 9)
		unk_0x0B0C9A0F9AAEB7F0(&Global_8371, 16);

	if (unk_0xE87F28FD4128D063())
		unk_0x0F15249D24BC5ADA(0);

	Global_21845 = 5;

	if (iParam0 == 1)
		unk_0x0B0C9A0F9AAEB7F0(&Global_8370, 30);
	else
		unk_0x8744D2E3FC95740E(&Global_8370, 30);

	if (!func_66())
		Global_20500.f_1 = 3;

	return;
}

BOOL func_66() // Position - 0x5577
{
	if (Global_20500.f_1 == 1 || Global_20500.f_1 == 0)
		return true;

	return false;
}

BOOL func_67() // Position - 0x559E
{
	return IS_BIT_SET(Global_1963795, 19);
}

void func_68() // Position - 0x55AD
{
	if (Global_20500.f_1 == 9 || Global_20500.f_1 == 10)
	{
		Global_21898 = 0;
		Global_21894 = 1;
	}

	return;
}

void func_69() // Position - 0x55D6
{
	int num;
	int num2;
	int num3;

	num = 0;
	num2 = 0;

	if (!unk_0x4FAFF4BCB7633475(unk_0x4A8C381C258A124D()))
		if (func_77() && unk_0xD6179D448A06A77F(unk_0x4A8C381C258A124D(), 0))
			num2 = 1;

	if (!Global_78689)
	{
		if (!unk_0x4FAFF4BCB7633475(unk_0x4A8C381C258A124D()) && num2 == 0)
		{
			num3 = unk_0xB204F40D393426B6(unk_0x4A8C381C258A124D(), 0, 1);
		
			if (Global_78689)
				if (num3 == 15 || num3 == 16 || num3 == 17 || num3 == 18 || num3 == 19 || unk_0x6DBAC05AFA907A23(unk_0x4A8C381C258A124D()))
					num = 1;
			else if (unk_0x6DBAC05AFA907A23(unk_0x4A8C381C258A124D()))
				num = 1;
		
			if (!Global_20443)
			{
				if (Global_7685 != 128)
				{
					if (num == 0)
					{
						if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
						{
							if (unk_0x7F420695E3F776FB(unk_0x4A8C381C258A124D(), 0))
							{
							}
							else
							{
								if (Global_78689)
								{
									unk_0x0428AFDCAA63B06E(unk_0x4A8C381C258A124D(), 244, 0);
									unk_0x0428AFDCAA63B06E(unk_0x4A8C381C258A124D(), 243, 0);
									unk_0x0428AFDCAA63B06E(unk_0x4A8C381C258A124D(), 242, 0);
								}
							
								unk_0x0B0C9A0F9AAEB7F0(&Global_8370, 11);
								unk_0x0974B2530CABDF56(unk_0x4A8C381C258A124D(), 1, 1);
							}
						}
					}
				}
			}
		}
	}

	return;
}

void func_70() // Position - 0x56FA
{
	if (IS_BIT_SET(Global_8370, 14) && Global_4542581 == 0 && Global_20440 == 0)
	{
		if (func_75())
		{
		}
		else
		{
			func_71();
		}
	
		if (Global_20500.f_1 == 9)
			if (Global_21858 == false)
				bLocal_75 = true;
	}

	return;
}

void func_71() // Position - 0x5742
{
	var unk;

	unk_0x78C4EBB0251847E2(&unk);

	if (Global_20453[Global_20445 /*3*/].f_1 == unk.f_1)
	{
	}
	else
	{
		Global_20440 = 1;
	}

	return;
}

void func_72() // Position - 0x5769
{
	if (IS_BIT_SET(Global_8370, 14) && Global_4542581 == 0 && Global_20440 == 0)
	{
		if (bLocal_75 == false)
		{
			if (IS_BIT_SET(Global_8371, 26))
			{
				unk_0x8744D2E3FC95740E(&Global_8371, 24);
				unk_0x8744D2E3FC95740E(&Global_8371, 25);
				unk_0x8744D2E3FC95740E(&Global_8371, 27);
				unk_0x8744D2E3FC95740E(&Global_8371, 26);
			
				if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
					if (!unk_0x7F420695E3F776FB(unk_0x4A8C381C258A124D(), 0))
						func_71();
					else if (func_74() == false)
						func_71();
			}
		}
	
		if (Global_20500.f_1 == 10 || Global_20500.f_1 == 9)
		{
			if (func_74())
				func_73();
		}
		else if (func_75())
		{
		}
		else if (func_74())
		{
			func_73();
		}
	
		if (Global_20500.f_1 == 9)
			if (Global_21858 == false)
				bLocal_75 = true;
	}

	return;
}

void func_73() // Position - 0x583F
{
	var unk;

	unk_0x78C4EBB0251847E2(&unk);

	if (Global_20446[Global_20445 /*3*/].f_1 == unk.f_1)
	{
	}
	else
	{
		Global_4542581 = 1;
	}

	return;
}

BOOL func_74() // Position - 0x5867
{
	int num;
	int num2;
	int num3;
	var unk;

	if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
	{
		if (Global_20500.f_1 == 10 || Global_20500.f_1 == 9)
			if (Global_20443 == false)
				if (Global_7685 != 128)
					if (!unk_0x7F420695E3F776FB(unk_0x4A8C381C258A124D(), 0))
						Global_21845 != 2;
	
		if (func_82(14))
			return false;
	
		if (unk_0xD6179D448A06A77F(unk_0x4A8C381C258A124D(), 0))
			return false;
	
		if (unk_0x7F420695E3F776FB(unk_0x4A8C381C258A124D(), 0))
			if (!unk_0x9CA0AFAD60564183())
				if (unk_0x69799E0840A34AFB(unk_0x6EF03BE64E058E2F(unk_0x4A8C381C258A124D(), 0)))
					return false;
	
		if (unk_0x2CFBD7757B4D922F(unk_0x4A8C381C258A124D()) || unk_0x9AC89B274C35B3FC(unk_0x4A8C381C258A124D()) || unk_0xB96BD3A32937D44E(unk_0x259BE71D8A81D4FA()) || unk_0xC3174723CF144560(unk_0x4A8C381C258A124D()) && !(IS_BIT_SET(Global_4718592.f_37, 17) && unk_0x834C960822A4683F() || Global_1927335))
			return false;
	
		if (Global_112858)
			return false;
	}

	if (Global_78689)
		return false;

	num3 = 0;
	num = unk_0xBF03D0685ADC793B();
	num2 = unk_0xBCF87EE3DC296C2A(num);

	if (num2 == 4 && num == 0 || unk_0x9CA0AFAD60564183())
	{
		num3 = 1;
	
		if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
		{
			if (unk_0x7F420695E3F776FB(unk_0x4A8C381C258A124D(), 0))
			{
				unk = unk_0x6EF03BE64E058E2F(unk_0x4A8C381C258A124D(), 0);
			
				if (unk_0x78B050AFBA6D1517(unk_0x4B423FAA24E8ABF0(unk)) || unk_0x3801E353091A2E42(unk_0x4B423FAA24E8ABF0(unk)) || unk_0x9910CD0D8626AA28(unk_0x4B423FAA24E8ABF0(unk)) || unk_0x4B423FAA24E8ABF0(unk) == joaat("seashark") || unk_0x4B423FAA24E8ABF0(unk) == joaat("seashark2") || unk_0x4B423FAA24E8ABF0(unk) == joaat("rhino") || unk_0x4B423FAA24E8ABF0(unk) == joaat("submersible") || unk_0x4B423FAA24E8ABF0(unk) == joaat("submersible2") || unk_0x4B423FAA24E8ABF0(unk) == joaat("toro"))
					num3 = 0;
			}
		}
	}

	if (Global_4542580 || num3 == 1)
		if (unk_0x486FF5D06E9659F1(joaat("apptrackify")) > 0 || Global_113810.f_14054.f_89)
			if (unk_0x486FF5D06E9659F1(joaat("michael2")) > 0)
				return true;
			else
				return false;
		else
			return true;

	return false;
}

BOOL func_75() // Position - 0x5AB0
{
	if (unk_0x486FF5D06E9659F1(joaat("appcamera")) > 0)
		return true;

	return false;
}

void func_76() // Position - 0x5ACD
{
	if (!Global_78689)
	{
		if (!unk_0x4FAFF4BCB7633475(unk_0x4A8C381C258A124D()))
		{
			if (IS_BIT_SET(Global_8370, 11))
			{
				if (!Global_20443)
					unk_0x0974B2530CABDF56(unk_0x4A8C381C258A124D(), 0, 1);
			
				if (Global_78689)
				{
					unk_0x0428AFDCAA63B06E(unk_0x4A8C381C258A124D(), 244, 1);
					unk_0x0428AFDCAA63B06E(unk_0x4A8C381C258A124D(), 243, 1);
					unk_0x0428AFDCAA63B06E(unk_0x4A8C381C258A124D(), 242, 1);
				}
			
				unk_0x8744D2E3FC95740E(&Global_8370, 11);
			}
		}
	}

	return;
}

BOOL func_77() // Position - 0x5B34
{
	var unk;
	int num;

	unk = unk_0xBF03D0685ADC793B();
	num = unk_0xBCF87EE3DC296C2A(unk);

	if (num == 4)
		return true;

	return false;
}

void func_78(char* sParam0, int iParam1) // Position - 0x5B58
{
	unk_0xAC98CA65AD9A3215(sParam0);
	unk_0x5E01B6B1F460FE3F(0, 0, 1, iParam1);
	return;
}

void func_79() // Position - 0x5B6F
{
	unk_0x90D1A8814E85B166(&iLocal_85);
	iLocal_85 == -1;
	return;
}

void func_80() // Position - 0x5B83
{
	if (Global_78689)
	{
		if (func_40() == false)
			return;
	
		func_81(Global_20481, "SET_SOFT_KEYS", 1f, 1f, 9f, -1f, -1f, 0, 0, 0, 0, 0);
		unk_0x0B0C9A0F9AAEB7F0(&Global_8370, 17);
	}

	return;
}

void func_81(var uParam0, char* sParam1, float fParam2, float fParam3, float fParam4, float fParam5, float fParam6, char* sParam7, char* sParam8, char* sParam9, char* sParam10, int iParam11) // Position - 0x5BBE
{
	unk_0x88F483FBD433696A(uParam0, sParam1);
	unk_0x330108B080A2132F(SYSTEM::ROUND(fParam2));

	if (fParam3 != -1f)
		unk_0x330108B080A2132F(SYSTEM::ROUND(fParam3));

	if (fParam4 != -1f)
		unk_0x330108B080A2132F(SYSTEM::ROUND(fParam4));

	if (fParam5 != -1f)
		unk_0x330108B080A2132F(SYSTEM::ROUND(fParam5));

	if (fParam6 != -1f)
		unk_0x330108B080A2132F(SYSTEM::ROUND(fParam6));

	if (!unk_0xD6F9DEE4765092A9(sParam7))
		func_36(sParam7);

	if (!unk_0xD6F9DEE4765092A9(sParam8))
		func_36(sParam8);

	if (!unk_0xD6F9DEE4765092A9(sParam9))
		func_36(sParam9);

	if (!unk_0xD6F9DEE4765092A9(sParam10))
		func_36(sParam10);

	if (!unk_0xD6F9DEE4765092A9(iParam11))
		func_36(iParam11);

	unk_0xE6B753D52F4CA222();
	return;
}

BOOL func_82(int iParam0) // Position - 0x5C71
{
	return Global_43377 == iParam0;
}

void func_83(int iParam0) // Position - 0x5C7F
{
	int i;
	int num;
	int num2;
	int j;
	int value;
	int k;
	int value2;
	int num3;
	int num4;
	int value3;

	Global_22913 = 0;
	Global_8975 = iParam0;

	for (i = 0; i < 9; i = i + 1)
	{
		Global_8939[i] = false;
	}

	for (i = 0; i < 9; i = i + 1)
	{
		num = 0;
	
		if (func_82(14))
		{
			while (num < 34)
			{
				if (iParam0 == Global_8377[num /*15*/].f_11)
				{
					if (i == Global_8377[num /*15*/].f_4)
					{
						if (Global_8939[i] == false)
						{
							Global_8903[i] = num;
						
							if (num == 3)
							{
								if (IS_BIT_SET(Global_8371, 3))
								{
									num2 = 42;
									Global_20708 = 1;
								}
								else
								{
									num2 = 255;
									Global_20708 = 0;
								}
							
								unk_0x88F483FBD433696A(Global_20481, "SET_DATA_SLOT");
								unk_0x330108B080A2132F(1);
								unk_0x330108B080A2132F(i);
								unk_0x330108B080A2132F(Global_8377[num /*15*/].f_10);
								unk_0x330108B080A2132F(0);
								func_36(&Global_8377[num /*15*/]);
								unk_0x330108B080A2132F(num2);
								unk_0xE6B753D52F4CA222();
							}
						
							if (Global_2694571)
								if (num == 14)
									func_81(Global_20481, "SET_DATA_SLOT", SYSTEM::TO_FLOAT(1), SYSTEM::TO_FLOAT(i), SYSTEM::TO_FLOAT(Global_8377[num /*15*/].f_10), SYSTEM::TO_FLOAT(Global_22908), -1f, &Global_8377[num /*15*/], 0, 0, 0, 0);
						
							Global_8939[i] = true;
						}
					}
				}
			
				num = num + 1;
			}
		}
		else
		{
			while (num < 34)
			{
				if (iParam0 == Global_8377[num /*15*/].f_11)
				{
					if (i == Global_8377[num /*15*/].f_4)
					{
						if (Global_8939[i] == false)
						{
							Global_8903[i] = num;
						
							if (num == 1)
							{
								for (j = 0; j < 35; j = j + 1)
								{
									if (Global_113810.f_14144[j /*104*/].f_24 != 0)
										if (Global_113810.f_14144[j /*104*/].f_28 == 0)
											if (Global_113810.f_14144[j /*104*/].f_99[Global_20500] == 1)
												Global_22913 = Global_22913 + 1;
								}
							
								func_81(Global_20481, "SET_DATA_SLOT", SYSTEM::TO_FLOAT(1), SYSTEM::TO_FLOAT(i), SYSTEM::TO_FLOAT(Global_8377[num /*15*/].f_10), SYSTEM::TO_FLOAT(Global_22913), -1f, &Global_8377[num /*15*/], 0, 0, 0, 0);
							}
							else if (num == 7)
							{
								if (Global_78689)
								{
									value = 0;
									value = Global_4541036;
								
									for (k = 0; k < 12; k = k + 1)
									{
										if (Global_4541037[k /*104*/].f_24 != 0)
											if (Global_4541037[k /*104*/].f_28 == 0)
												if (Global_4541037[k /*104*/].f_99[Global_20500] == 1)
													value = value + 1;
									}
								
									func_81(Global_20481, "SET_DATA_SLOT", SYSTEM::TO_FLOAT(1), SYSTEM::TO_FLOAT(i), SYSTEM::TO_FLOAT(Global_8377[num /*15*/].f_10), SYSTEM::TO_FLOAT(value), -1f, &Global_8377[num /*15*/], 0, 0, 0, 0);
								}
								else
								{
									switch (Global_20500)
									{
										case 0:
											value2 = Global_44580;
											break;
									
										case 1:
											value2 = Global_44581;
											break;
									
										case 2:
											value2 = Global_44582;
											break;
									
										default:
											break;
									}
								
									func_81(Global_20481, "SET_DATA_SLOT", SYSTEM::TO_FLOAT(1), SYSTEM::TO_FLOAT(i), SYSTEM::TO_FLOAT(Global_8377[num /*15*/].f_10), SYSTEM::TO_FLOAT(value2), -1f, &Global_8377[num /*15*/], 0, 0, 0, 0);
								}
							}
							else if (num == 14)
							{
								func_81(Global_20481, "SET_DATA_SLOT", SYSTEM::TO_FLOAT(1), SYSTEM::TO_FLOAT(i), SYSTEM::TO_FLOAT(Global_8377[num /*15*/].f_10), SYSTEM::TO_FLOAT(Global_22908), -1f, &Global_8377[num /*15*/], 0, 0, 0, 0);
							}
							else if (num == 20)
							{
								unk_0x88F483FBD433696A(Global_20481, "SET_DATA_SLOT");
								unk_0x330108B080A2132F(1);
								unk_0x330108B080A2132F(i);
								unk_0x330108B080A2132F(Global_8377[num /*15*/].f_10);
								unk_0x330108B080A2132F(0);
								func_36(&Global_8377[num /*15*/]);
								unk_0x330108B080A2132F(Global_8376);
								unk_0xE6B753D52F4CA222();
							}
							else if (num == 2)
							{
								if (IS_BIT_SET(Global_8371, 6))
									num3 = 42;
								else
									num3 = 255;
							
								unk_0x88F483FBD433696A(Global_20481, "SET_DATA_SLOT");
								unk_0x330108B080A2132F(1);
								unk_0x330108B080A2132F(i);
								unk_0x330108B080A2132F(Global_8377[num /*15*/].f_10);
								unk_0x330108B080A2132F(0);
								func_36(&Global_8377[num /*15*/]);
								unk_0x330108B080A2132F(num3);
								unk_0xE6B753D52F4CA222();
							}
							else if (num == 3)
							{
								if (IS_BIT_SET(Global_8371, 3))
								{
									num4 = 42;
									Global_20708 = 1;
								}
								else
								{
									num4 = 255;
									Global_20708 = 0;
								}
							
								unk_0x88F483FBD433696A(Global_20481, "SET_DATA_SLOT");
								unk_0x330108B080A2132F(1);
								unk_0x330108B080A2132F(i);
								unk_0x330108B080A2132F(Global_8377[num /*15*/].f_10);
								unk_0x330108B080A2132F(0);
								func_36(&Global_8377[num /*15*/]);
								unk_0x330108B080A2132F(num4);
								unk_0xE6B753D52F4CA222();
							}
							else if (num == 8)
							{
								unk_0x88F483FBD433696A(Global_20481, "SET_DATA_SLOT");
								unk_0x330108B080A2132F(1);
								unk_0x330108B080A2132F(i);
								unk_0x330108B080A2132F(Global_8377[num /*15*/].f_10);
								unk_0x330108B080A2132F(0);
								func_36(&Global_8377[num /*15*/]);
								unk_0x330108B080A2132F(42);
								unk_0xE6B753D52F4CA222();
							}
							else if (num == 23 && unk_0x1B79E937E91F40C3(&Global_8377[num /*15*/], "CELL_BENWEB" /*Benny's*/) && IS_BIT_SET(Global_8371, 6))
							{
								unk_0x88F483FBD433696A(Global_20481, "SET_DATA_SLOT");
								unk_0x330108B080A2132F(1);
								unk_0x330108B080A2132F(i);
								unk_0x330108B080A2132F(Global_8377[num /*15*/].f_10);
								unk_0x330108B080A2132F(0);
								func_36(&Global_8377[num /*15*/]);
								unk_0x330108B080A2132F(42);
								unk_0xE6B753D52F4CA222();
							}
							else if (Global_8377[num /*15*/].f_10 == 57 && num == 23)
							{
								value3 = 0;
								value3 = Global_1890239.f_1;
								func_81(Global_20481, "SET_DATA_SLOT", SYSTEM::TO_FLOAT(1), SYSTEM::TO_FLOAT(i), SYSTEM::TO_FLOAT(Global_8377[num /*15*/].f_10), SYSTEM::TO_FLOAT(value3), -1f, &Global_8377[num /*15*/], 0, 0, 0, 0);
							}
							else
							{
								func_81(Global_20481, "SET_DATA_SLOT", SYSTEM::TO_FLOAT(1), SYSTEM::TO_FLOAT(i), SYSTEM::TO_FLOAT(Global_8377[num /*15*/].f_10), SYSTEM::TO_FLOAT(0), -1f, &Global_8377[num /*15*/], 0, 0, 0, 0);
							}
						
							Global_8939[i] = true;
						}
					}
				}
			
				num = num + 1;
			}
		}
	}

	return;
}

void func_84() // Position - 0x6219
{
	if (Global_78689 == false)
	{
		Global_8377[14 /*15*/].f_4 = -99;
		Global_8377[4 /*15*/].f_4 = -99;
	
		if (Global_2694571)
		{
			if (func_82(14))
			{
				func_37(2, "CELL_2" /*Internet*/, 2, "appInternet", 6, 1, 1, 0, 0);
				func_37(14, "CELL_29" /*Job List*/, 7, "appMPJobListNEW", 12, 1, 1, 0, 0);
			}
			else
			{
				func_37(14, "CELL_29" /*Job List*/, 2, "appMPJobListNEW", 12, 1, 1, 0, 0);
			}
		}
		else
		{
			func_37(4, "CELL_23" /*Check List*/, 2, "appChecklist", 39, 1, 1, 0, 0);
		}
	}

	return;
}

void func_85() // Position - 0x62A7
{
	int i;
	int num;
	int num2;

	for (i = 0; i < 34; i = i + 1)
	{
		Global_8377[i /*15*/].f_4 = -99;
	}

	if (Global_78689 == false)
	{
		num = 0;
	
		while (num2 < 161)
		{
			if (func_86(num2, Global_20500) == 1)
				num = 1;
		
			num2 = num2 + 1;
		}
	
		func_37(7, "CELL_5" /*Email*/, 0, "appEmail", 4, 1, 1, 0, 0);
		func_37(1, "CELL_1" /*Texts*/, 1, "appTextMessage", 2, 1, 1, 0, 0);
		func_37(4, "CELL_23" /*Check List*/, 2, "appChecklist", 39, 1, 1, 0, 0);
	
		if (Global_43377 == 15 && func_64(false) == false && Global_8375 == 0)
		{
			func_37(20, "CELL_32" /*Quick Save*/, 3, "appSettings", 43, 1, 1, 0, 0);
			Global_20707 = 0;
			Global_8376 = 255;
		}
		else
		{
			func_37(20, "CELL_32" /*Quick Save*/, 3, "appSettings", 43, 1, 1, 0, 0);
			Global_20707 = 1;
			Global_8376 = 42;
		}
	
		if (num == 1)
			func_37(0, "CELL_0" /*Contacts*/, 4, "appContacts", 27, 1, 1, 0, 0);
		else
			func_37(0, "CELL_0" /*Contacts*/, 4, "appContacts", 5, 1, 1, 0, 0);
	
		func_37(10, "CELL_16" /*Settings*/, 5, "appSettings", 24, 1, 1, 0, 0);
		func_37(3, "CELL_7" /*Snapmatic*/, 6, "appCamera", 1, 1, 1, 0, 0);
		func_37(2, "CELL_2" /*Internet*/, 7, "appInternet", 6, 1, 1, 0, 0);
	
		if (Global_113810.f_14054.f_89 == true)
			func_37(17, "CELL_28" /*Trackify*/, 8, "appTrackify", 42, 1, 1, 0, 0);
	
		if (Global_113810.f_14054.f_88 == 1)
			func_37(16, "CELL_25" /*Sniper*/, 0, "appContacts", 40, 2, 1, 0, 0);
	
		func_37(25, "CELL_14" /*More Apps*/, -99, "appContacts", 17, 2, 1, 0, 0);
		func_37(9, "CELL_13" /*BAWSAQ*/, -99, "appContacts", 13, 2, 1, 0, 0);
		func_37(11, "CELL_14" /*More Apps*/, -99, "appContacts", 8, 2, 1, 0, 0);
		func_37(27, "CELL_15" /*Spare*/, -99, "appContacts", 17, 2, 1, 0, 0);
		func_37(28, "CELL_15" /*Spare*/, -99, "appContacts", 17, 2, 1, 0, 0);
		func_37(29, "CELL_15" /*Spare*/, -99, "appSettings", 17, 2, 1, 0, 0);
		func_37(30, "CELL_15" /*Spare*/, -99, "appContacts", 17, 2, 1, 0, 0);
		func_37(31, "CELL_15" /*Spare*/, -99, "appContacts", 17, 2, 1, 0, 0);
		func_37(32, "CELL_15" /*Spare*/, -99, "appSettings", 17, 2, 1, 0, 0);
	}
	else
	{
		func_37(7, "CELL_5" /*Email*/, 0, "appMPEmail", 4, 1, 1, 0, 0);
		func_37(10, "CELL_16" /*Settings*/, 5, "appSettings", 24, 1, 1, 0, 0);
		func_37(1, "CELL_1" /*Texts*/, 1, "appTextMessage", 2, 1, 1, 0, 0);
		func_37(3, "CELL_7" /*Snapmatic*/, 6, "appCamera", 1, 1, 1, 0, 0);
		func_37(2, "CELL_2" /*Internet*/, 7, "appInternet", 6, 1, 1, 0, 0);
		func_37(14, "CELL_29" /*Job List*/, 4, "appMPJobListNEW", 12, 1, 1, 0, 0);
		func_37(0, "CELL_0" /*Contacts*/, 2, "appContacts", 5, 1, 1, 0, 0);
		func_37(21, "CELL_37" /*Quick Join*/, 3, "AppJIPMP", 14, 1, 1, 0, 0);
	
		if (IS_BIT_SET(Global_4542302, 4) == true)
			func_37(17, "CELL_28" /*Trackify*/, 8, "appTrackify", 42, 1, 1, 0, 0);
	
		func_37(13, "CELL_35" /*Player List*/, -99, "appMPCopBackup", 39, 1, 1, 0, 0);
		func_37(13, "CELL_20", -99, "appMPCopBackup", 16, 1, 1, 0, 0);
		func_37(15, "CELL_18" /*Map*/, -99, "appContacts", 8, 1, 1, 0, 0);
		func_37(9, "CELL_13" /*BAWSAQ*/, -99, "appContacts", 13, 2, 1, 0, 0);
		func_37(5, "CELL_4" /*Sidetasks*/, -99, "appContacts", 12, 2, 1, 0, 0);
		func_37(23, "CELL_15" /*Spare*/, 0, "appContacts", 17, 2, 1, 0, 0);
		func_37(24, "CELL_15" /*Spare*/, 1, "appContacts", 17, 2, 1, 0, 0);
		func_37(25, "CELL_15" /*Spare*/, 2, "appContacts", 17, 2, 1, 0, 0);
		func_37(26, "CELL_15" /*Spare*/, 3, "appContacts", 17, 2, 1, 0, 0);
		func_37(27, "CELL_15" /*Spare*/, 4, "appContacts", 17, 2, 1, 0, 0);
		func_37(28, "CELL_15" /*Spare*/, 5, "appContacts", 17, 2, 1, 0, 0);
		func_37(29, "CELL_15" /*Spare*/, 6, "appContacts", 17, 2, 1, 0, 0);
		func_37(30, "CELL_15" /*Spare*/, 7, "appContacts", 17, 2, 1, 0, 0);
		func_37(31, "CELL_15" /*Spare*/, 8, "appContacts", 17, 2, 1, 0, 0);
		func_37(32, "CELL_15" /*Spare*/, 9, "appContacts", 17, 2, 1, 0, 0);
		func_37(33, "CELL_15" /*Spare*/, 10, "appContacts", 17, 2, 1, 0, 0);
	
		if (!IS_BIT_SET(Global_4542302, 4) == true)
			if (Global_1836104)
				func_37(23, "CELL_CIRCBREAK" /*VLSI Unlock*/, 8, "AppVLSI", 54, 1, 1, 0, 0);
			else if (IS_BIT_SET(Global_4542302, 20) == true)
				func_37(23, "CELL_SIGHTS" /*Sightseer*/, 8, "AppSettings", 59, 1, 1, 0, 0);
			else if (IS_BIT_SET(Global_4542302, 22) == true)
				func_37(23, "CELL_EXTRACT" /*Extraction*/, 8, "AppExtraction", 58, 1, 1, 0, 0);
			else if (IS_BIT_SET(Global_4542302, 26) == true)
				if (func_122())
					func_37(23, "CELL_0" /*Contacts*/, -99, "appContacts", 5, 1, 1, 0, 0);
				else
					func_37(23, "CELL_SECHACK" /*SecuroServ Hack*/, 8, "AppSecuroHack", 57, 1, 1, 0, 0);
	
		if (IS_BIT_SET(Global_4542302, 4) == false && Global_1836104 == false && IS_BIT_SET(Global_4542302, 20) == false && IS_BIT_SET(Global_4542302, 22) == false && IS_BIT_SET(Global_4542302, 26) == false)
			if (func_122())
				func_37(23, "CELL_0" /*Contacts*/, -99, "appContacts", 5, 1, 1, 0, 0);
			else
				func_37(23, "CELL_BOSSAGE" /*SecuroServ*/, 8, "appMPBossAgency", 57, 1, 1, 0, 0);
	}

	return;
}

int func_86(int iParam0, int iParam1) // Position - 0x6857
{
	if (iParam1 < 0 || iParam1 > 4)
		return 0;

	return Global_2058[iParam0 /*29*/].f_19[iParam1];
}

BOOL func_87(int iParam0) // Position - 0x6881
{
	int num;

	iParam0 == 1;

	if (iParam0 == 0)
		if (func_90() == 0)
			return false;

	if (func_88(&num) && unk_0x38640D2193CB547F(903) == 1)
		return true;

	if (!func_88(&num))
		if (num == 6)
			return true;
		else if (num == 7)
			return true;
		else if (num == 8)
			return true;
		else if (num == 10)
			return true;
		else if (num == 12)
			return true;
		else if (num == 13)
			return true;
		else if (num == 11)
			return true;

	return false;
}

BOOL func_88(var uParam0) // Position - 0x6924
{
	if (func_89())
	{
		*uParam0 = 11;
		return true;
	}

	return unk_0x0C81430427CA20D7(uParam0);
}

BOOL func_89() // Position - 0x6942
{
	return Global_32561;
}

int func_90() // Position - 0x694D
{
	if (func_91())
	{
		if (!unk_0xECBFC40AE72FA489())
		{
			if (unk_0x444B1E9E9A9D726B() == 4)
				return 0;
		
			if (unk_0x444B1E9E9A9D726B() == 2)
				return 0;
		
			if (unk_0x444B1E9E9A9D726B() == 1)
				return 0;
		
			if (unk_0x444B1E9E9A9D726B() == 5)
				return 0;
		}
	}

	return 1;
}

BOOL func_91() // Position - 0x6995
{
	return unk_0x3B880DE16766E9C3() || unk_0x716271729B9FB8E6();
}

void func_92() // Position - 0x69AB
{
	if (unk_0x75EAB09F5E974116(unk_0x259BE71D8A81D4FA()))
		iLocal_114 = unk_0x148523145C045769(unk_0x3348018F21E261AF(unk_0xD1A6A821F5AC81DB(unk_0x4A8C381C258A124D(), 1)));

	switch (iLocal_114)
	{
		case 0:
			iLocal_89 = 5;
			break;
	
		case 1:
			iLocal_89 = 5;
			break;
	
		case 2:
			iLocal_89 = 4;
			break;
	
		case 3:
			iLocal_89 = 4;
			break;
	
		case 4:
			iLocal_89 = 3;
			break;
	
		case 5:
			iLocal_89 = 2;
			break;
	
		default:
			iLocal_89 = 3;
			break;
	}

	iLocal_115 = unk_0xC5935DFB3F39785A(0, 8);

	if (iLocal_115 < 2)
		if (iLocal_89 > 2)
			iLocal_89 = iLocal_89 - 1;

	if (bLocal_116 == true || func_93())
		iLocal_89 = 0;

	func_113(Global_20481, "SET_PROVIDER_ICON", SYSTEM::TO_FLOAT(iLocal_88), SYSTEM::TO_FLOAT(iLocal_89), -1082130432, -1082130432, -1082130432);
	return;
}

BOOL func_93() // Position - 0x6A7B
{
	int num;
	var unk;
	int i;

	if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
	{
		num = func_99();
	
		if (func_98(num))
		{
			unk = { unk_0xD1A6A821F5AC81DB(unk_0x4A8C381C258A124D(), 1) };
		
			for (i = 0; i < Global_113810.f_7691.f_136; i = i + 1)
			{
				if (IS_BIT_SET(Global_113810.f_7691[i /*15*/].f_2, num))
					if (func_96(unk, func_97(i)))
						return true;
			}
		
			for (i = 0; i < Global_113810.f_7691.f_764; i = i + 1)
			{
				if (IS_BIT_SET(Global_113810.f_7691.f_651[i /*14*/].f_2, num))
					if (func_96(unk, func_95(i)))
						return true;
			}
		
			for (i = 0; i < Global_113810.f_7691.f_866; i = i + 1)
			{
				if (IS_BIT_SET(Global_113810.f_7691.f_765[i /*10*/].f_2, num))
					if (func_96(unk, func_94(i)))
						return true;
			}
		}
	}

	return false;
}

int func_94(int iParam0) // Position - 0x6B7B
{
	return Global_113810.f_7691.f_765[iParam0 /*10*/].f_7;
}

int func_95(int iParam0) // Position - 0x6B93
{
	return Global_113810.f_7691.f_651[iParam0 /*14*/].f_7;
}

BOOL func_96(Vector3 vParam0, var uParam1, var uParam2, int iParam3) // Position - 0x6BAB
{
	if (iParam3 != -1)
	{
		if (iParam3 >= Global_43378)
			return false;
	
		if (SYSTEM::VDIST2(vParam0, Global_43378[iParam3 /*5*/]) <= Global_43378[iParam3 /*5*/].f_3 * Global_43378[iParam3 /*5*/].f_3)
			return true;
		else if (Global_43378[iParam3 /*5*/].f_4 != -1)
			return func_96(vParam0, Global_43378[iParam3 /*5*/].f_4);
	}

	return false;
}

int func_97(int iParam0) // Position - 0x6C16
{
	return Global_113810.f_7691[iParam0 /*15*/].f_7;
}

BOOL func_98(int iParam0) // Position - 0x6C2B
{
	return iParam0 < 3;
}

int func_99() // Position - 0x6C37
{
	func_100();
	return Global_113810.f_2366.f_539.f_4321;
}

void func_100() // Position - 0x6C50
{
	int num;

	if (unk_0xFC8BFE4B41177C22(unk_0x4A8C381C258A124D()))
	{
		if (func_102(Global_113810.f_2366.f_539.f_4321) != unk_0x4B423FAA24E8ABF0(unk_0x4A8C381C258A124D()))
		{
			num = func_101(unk_0x4A8C381C258A124D());
		
			if (func_98(num) && !func_82(14) || Global_112760)
			{
				if (Global_113810.f_2366.f_539.f_4321 != num && func_98(Global_113810.f_2366.f_539.f_4321))
					Global_113810.f_2366.f_539.f_4322 = Global_113810.f_2366.f_539.f_4321;
			
				Global_113810.f_2366.f_539.f_4323 = num;
				Global_113810.f_2366.f_539.f_4321 = num;
				return;
			}
		}
		else
		{
			if (Global_113810.f_2366.f_539.f_4321 != 145)
				Global_113810.f_2366.f_539.f_4323 = Global_113810.f_2366.f_539.f_4321;
		
			return;
		}
	}

	Global_113810.f_2366.f_539.f_4321 = 145;
	return;
}

int func_101(var uParam0) // Position - 0x6D4D
{
	int i;
	int num;

	if (unk_0xFC8BFE4B41177C22(uParam0))
	{
		num = unk_0x4B423FAA24E8ABF0(uParam0);
	
		for (i = 0; i <= 2; i = i + 1)
		{
			if (func_102(i) == num)
				return i;
		}
	}

	return 145;
}

int func_102(int iParam0) // Position - 0x6D8A
{
	if (func_98(iParam0))
		return func_103(iParam0);
	else
		iParam0 != 145;

	return 0;
}

int func_103(int iParam0) // Position - 0x6DAF
{
	return Global_2058[iParam0 /*29*/];
}

int func_104() // Position - 0x6DBE
{
	func_123();
	return Global_113810.f_14054[Global_20500 /*20*/].f_8;
}

struct<13> func_105(var uParam0) // Position - 0x6DD8
{
	var unk;

	unk_0x464E131FD68B953F(uParam0, &unk, 13);
	return unk;
}

void func_106() // Position - 0x6DEF
{
	if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
	{
		if (Global_20500 == 0)
		{
			switch (Global_113810.f_14054[Global_20500 /*20*/].f_6)
			{
				case 1:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 0);
					break;
			
				case 2:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 1);
					break;
			
				case 3:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 2);
					break;
			
				case 4:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 3);
					break;
			
				case 5:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 4);
					break;
			
				case 6:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 5);
					break;
			
				case 7:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 6);
					break;
			
				default:
					break;
			}
		}
	
		if (Global_20500 == 1)
		{
			switch (Global_113810.f_14054[Global_20500 /*20*/].f_6)
			{
				case 1:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 6);
					break;
			
				case 2:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 0);
					break;
			
				case 3:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 1);
					break;
			
				case 4:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 2);
					break;
			
				case 5:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 3);
					break;
			
				case 6:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 4);
					break;
			
				case 7:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 5);
					break;
			
				default:
					break;
			}
		}
	
		if (Global_20500 == 2)
		{
			switch (Global_113810.f_14054[Global_20500 /*20*/].f_6)
			{
				case 1:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 5);
					break;
			
				case 2:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 6);
					break;
			
				case 3:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 1);
					break;
			
				case 4:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 0);
					break;
			
				case 5:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 2);
					break;
			
				case 6:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 3);
					break;
			
				case 7:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 4);
					break;
			
				default:
					break;
			}
		}
	
		if (Global_20500 == 3)
		{
			switch (Global_4542578)
			{
				case 1:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 0);
					break;
			
				case 2:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 1);
					break;
			
				case 3:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 2);
					break;
			
				case 4:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 3);
					break;
			
				case 5:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 4);
					break;
			
				case 6:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 5);
					break;
			
				case 7:
					unk_0x454108124161B7D7(unk_0x259BE71D8A81D4FA(), 6);
					break;
			
				default:
					break;
			}
		}
	}

	return;
}

int func_107(int iParam0, int iParam1) // Position - 0x7066
{
	var unk;
	int num;

	if (iParam0 != 14385)
	{
		unk = func_108(iParam0, iParam1);
	
		if (unk_0xDF7F16323520B858(unk, &num, -1))
			return num;
	}

	return 0;
}

var func_108(int iParam0, int iParam1) // Position - 0x7095
{
	return unk_0xD69CE161FE614531(0, iParam0, func_109(iParam1));
}

int func_109(int iParam0) // Position - 0x70AA
{
	int num;
	int num2;

	num = iParam0;

	if (num == -1)
	{
		num2 = func_110();
	
		if (num2 > -1)
		{
			Global_2805862 = 0;
			num = num2;
		}
		else
		{
			num = 0;
			Global_2805862 = 1;
		}
	}

	return num;
}

int func_110() // Position - 0x70DE
{
	return Global_1574918;
}

void func_111() // Position - 0x70EA
{
	if (func_82(14))
	{
		if (Global_2694571)
		{
			if (Global_20500.f_1 == 6)
			{
				if (Global_20480 == 7)
				{
					func_113(Global_20481, "DISPLAY_VIEW", 1f, SYSTEM::TO_FLOAT(7), -1082130432, -1082130432, -1082130432);
				}
				else
				{
					Global_20480 = 6;
					func_113(Global_20481, "DISPLAY_VIEW", 1f, SYSTEM::TO_FLOAT(6), -1082130432, -1082130432, -1082130432);
				}
			}
		}
		else if (Global_20500.f_1 == 6)
		{
			func_113(Global_20481, "DISPLAY_VIEW", 1f, SYSTEM::TO_FLOAT(6), -1082130432, -1082130432, -1082130432);
		}
	}

	iLocal_91 = unk_0x18E502A71E28968C();

	if (iLocal_91 != Global_8373)
	{
		Global_8373 = iLocal_91;
		iLocal_90 = unk_0x5295501D0862870D();
		iLocal_92 = unk_0xDDA4278694CAA369();
	
		switch (iLocal_92)
		{
			case 0:
				TEXT_LABEL_ASSIGN_STRING(&uLocal_93, "CELL_920" /*Sun*/, 16);
				break;
		
			case 1:
				TEXT_LABEL_ASSIGN_STRING(&uLocal_93, "CELL_921" /*Mon*/, 16);
				break;
		
			case 2:
				TEXT_LABEL_ASSIGN_STRING(&uLocal_93, "CELL_922" /*Tue*/, 16);
				break;
		
			case 3:
				TEXT_LABEL_ASSIGN_STRING(&uLocal_93, "CELL_923" /*Wed*/, 16);
				break;
		
			case 4:
				TEXT_LABEL_ASSIGN_STRING(&uLocal_93, "CELL_924" /*Thu*/, 16);
				break;
		
			case 5:
				TEXT_LABEL_ASSIGN_STRING(&uLocal_93, "CELL_925" /*Fri*/, 16);
				break;
		
			case 6:
				TEXT_LABEL_ASSIGN_STRING(&uLocal_93, "CELL_926" /*Sat*/, 16);
				break;
		
			default:
				TEXT_LABEL_ASSIGN_STRING(&uLocal_93, "CELL_206" /*BACK*/, 16);
				break;
		}
	
		func_81(Global_20481, "SET_TITLEBAR_TIME", SYSTEM::TO_FLOAT(iLocal_90), SYSTEM::TO_FLOAT(iLocal_91), -1f, -1f, -1f, &uLocal_93, 0, 0, 0, 0);
	
		if (Global_20443 == false)
			func_92();
	
		if (unk_0x75EAB09F5E974116(unk_0x259BE71D8A81D4FA()))
			uLocal_99 = unk_0x8D91ADE44AC79BC9(unk_0x4A8C381C258A124D());
	}

	return;
}

void func_112() // Position - 0x726A
{
	if (Global_78689)
	{
		TEXT_LABEL_ASSIGN_STRING(&Global_20489, "Phone_SoundSet_Default", 24);
		unk_0x531FF8BCC6FFD9E3(0);
	}
	else if (Global_20443)
	{
		unk_0x531FF8BCC6FFD9E3(4);
		TEXT_LABEL_ASSIGN_STRING(&Global_20489, "Phone_SoundSet_Prologue", 24);
	}
	else
	{
		switch (Global_20500)
		{
			case 0:
				TEXT_LABEL_ASSIGN_STRING(&Global_20489, "Phone_SoundSet_Michael", 24);
				unk_0x531FF8BCC6FFD9E3(0);
				break;
		
			case 2:
				TEXT_LABEL_ASSIGN_STRING(&Global_20489, "Phone_SoundSet_Trevor", 24);
				unk_0x531FF8BCC6FFD9E3(1);
				break;
		
			case 1:
				TEXT_LABEL_ASSIGN_STRING(&Global_20489, "Phone_SoundSet_Franklin", 24);
				unk_0x531FF8BCC6FFD9E3(2);
				break;
		
			default:
				TEXT_LABEL_ASSIGN_STRING(&Global_20489, "Phone_SoundSet_Default", 24);
				unk_0x531FF8BCC6FFD9E3(0);
				break;
		}
	}

	Global_20463 = { -90f, -130f, 0f };

	if (unk_0xC003EE4BA64FE66C())
	{
		Global_20445 = 0;
		Global_20446[0 /*3*/] = { unk_0x897B441FF18ECBBB() * 117.2f, unk_0x897B441FF18ECBBB() * -158.8f, -113f };
		Global_20453[0 /*3*/] = { unk_0x897B441FF18ECBBB() * 117.2f, unk_0x897B441FF18ECBBB() * -53.3f, -113f };
	}
	else
	{
		Global_20445 = 0;
		Global_20446[0 /*3*/] = { unk_0x897B441FF18ECBBB() * 85.7f, unk_0x897B441FF18ECBBB() * -121.8f, -91.5f };
		Global_20453[0 /*3*/] = { unk_0x897B441FF18ECBBB() * 85.7f, unk_0x897B441FF18ECBBB() * -35.3f, -91.5f };
	}

	Global_20428 = { Global_20446[Global_20445 /*3*/] };
	unk_0x6B7EA0158D00C600(Global_20446[Global_20445 /*3*/]);
	unk_0xA0382000AA987093(Global_20463, 0);
	Global_20440 = 1;
	return;
}

void func_113(var uParam0, char* sParam1, float fParam2, int iParam3, int iParam4, int iParam5, int iParam6) // Position - 0x73CF
{
	unk_0x88F483FBD433696A(uParam0, sParam1);
	unk_0x330108B080A2132F(SYSTEM::ROUND(fParam2));

	if (iParam3 != -1f)
		unk_0x330108B080A2132F(SYSTEM::ROUND(iParam3));

	if (iParam4 != -1f)
		unk_0x330108B080A2132F(SYSTEM::ROUND(iParam4));

	if (iParam5 != -1f)
		unk_0x330108B080A2132F(SYSTEM::ROUND(iParam5));

	if (iParam6 != -1f)
		unk_0x330108B080A2132F(SYSTEM::ROUND(iParam6));

	unk_0xE6B753D52F4CA222();
	return;
}

BOOL func_114() // Position - 0x7432
{
	return unk_0xFD5A25A8B9488D42() || unk_0x6823557BDD18031C();
}

void func_115() // Position - 0x7448
{
	Global_2801145.f_1 = 0;
	Global_2801145 = 0;
	Global_2801145.f_2 = 0;
	Global_2801145.f_33 = -1;
	Global_2801145.f_34 = -1;
	TEXT_LABEL_ASSIGN_STRING(&(Global_2801145.f_4), "", 64);
	TEXT_LABEL_ASSIGN_STRING(&Global_2801145.f_39[0 /*16*/], "", 64);
	Global_2801145.f_38 = 0;
	Global_2801145.f_56 = 0;
	Global_2801145.f_57 = 0;
	Global_2801145.f_58 = -2;
	Global_2801145.f_3 = 0;
	func_116(&(Global_2801145.f_20));
	return;
}

void func_116(var uParam0) // Position - 0x74BB
{
	*uParam0 = 0;
	uParam0->f_1 = 0;
	uParam0->f_2 = 0;
	uParam0->f_3 = 0;
	uParam0->f_4 = 0;
	uParam0->f_5 = 0;
	uParam0->f_6 = 0;
	uParam0->f_7 = 0;
	uParam0->f_8 = 0;
	uParam0->f_9 = 0;
	uParam0->f_10 = 0;
	uParam0->f_11 = 0;
	uParam0->f_12 = 0;
	return;
}

void func_117() // Position - 0x7503
{
	Global_2801145.f_2 = 1;
	Global_2801145.f_38 = 1;

	if (unk_0x261E3728EE56B3AC())
	{
		if (unk_0x5150DDF9D3BDA9E5())
		{
			while (unk_0x8BDB5F6FED3994AD())
			{
				SYSTEM::WAIT(0);
			}
		
			unk_0x7D8697356CC73FFD();
			Global_2801145 = 0;
			Global_2801145.f_2 = 0;
		}
		else if (func_118(Global_2801145.f_20))
		{
			if (unk_0xEDD63461767A518C(&(Global_2801145.f_20)))
				if (!unk_0x484CF81C32C3D413(&(Global_2801145.f_20)))
					func_115();
		}
		else
		{
			func_115();
		}
	}
	else
	{
		func_115();
	}

	if (Global_2801145.f_37)
		func_65(0);

	Global_2801145.f_37 = 0;
	Global_2801145.f_3 = 0;
	return;
}

BOOL func_118(var uParam0, var uParam1, var uParam2, var uParam3, var uParam4, var uParam5, var uParam6, var uParam7, var uParam8, var uParam9, var uParam10, var uParam11, var uParam12) // Position - 0x75A3
{
	return unk_0xA8F635A578C0CE07(&uParam0, 13);
}

void func_119() // Position - 0x75B3
{
	Global_20711 = 0;
	func_10();
	return;
}

void func_120(var uParam0, char* sParam1) // Position - 0x75C3
{
	unk_0x88F483FBD433696A(uParam0, sParam1);
	unk_0xE6B753D52F4CA222();
	return;
}

void func_121() // Position - 0x75D8
{
	unk_0x4CC1CF98851922CE(0, Global_20469);
	unk_0x66EFB3D6110055C4(0, 70, 1);
	unk_0x66EFB3D6110055C4(0, 69, 1);

	if (unk_0xAE231F549813BBDF(2))
	{
		unk_0x4CC1CF98851922CE(0, Global_20469);
		unk_0x4CC1CF98851922CE(0, Global_20470);
		unk_0x66EFB3D6110055C4(0, 24, 1);
		unk_0x66EFB3D6110055C4(0, 257, 1);
	}

	if (Global_78689)
		Global_113810.f_14054[3 /*20*/].f_10 = func_107(1198, -1);

	if (unk_0x761778199FE1211C())
	{
		if (unk_0xD130E7CDEE903624(unk_0x4A8C381C258A124D(), "Synched"))
		{
		}
		else if (unk_0x1DD05E817C89C737() - iLocal_117 >= 300 || iLocal_117 == 0 || iLocal_117 > unk_0x1DD05E817C89C737())
		{
			unk_0x97A041099E92C69F(0, 100, 100);
			iLocal_117 = unk_0x1DD05E817C89C737();
		}
	}
	else if (Global_113810.f_14054[Global_20500 /*20*/].f_10 == 1)
	{
		if (unk_0x1DD05E817C89C737() - iLocal_117 >= 300 || iLocal_117 == 0 || iLocal_117 > unk_0x1DD05E817C89C737())
		{
			unk_0x97A041099E92C69F(0, 100, 100);
			iLocal_117 = unk_0x1DD05E817C89C737();
		}
	}

	if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
	{
		if (!unk_0x129DE7ED6456C4CA(unk_0x4A8C381C258A124D()))
		{
			if (Global_78689)
			{
				if (!unk_0x1B79E937E91F40C3(&Global_9175[3 /*2811*/][1 /*281*/].f_144[func_107(1199, -1) /*6*/], "Silent Ringtone Dummy"))
					unk_0x1BC60757AC20ACCF(&Global_9175[3 /*2811*/][1 /*281*/].f_144[func_107(1199, -1) /*6*/], unk_0x4A8C381C258A124D(), 1);
			}
			else if (!unk_0x1B79E937E91F40C3(&(Global_113810.f_14054[Global_20500 /*20*/].f_11), "Silent Ringtone Dummy"))
			{
				if (!(Global_9175[Global_20500 /*2811*/][0 /*281*/].f_259 == 1))
				{
					if (unk_0x761778199FE1211C())
					{
						if (unk_0xD130E7CDEE903624(unk_0x4A8C381C258A124D(), "Synched"))
						{
						}
						else
						{
							unk_0x1BC60757AC20ACCF(&(Global_113810.f_14054[Global_20500 /*20*/].f_11), unk_0x4A8C381C258A124D(), 1);
						}
					}
					else
					{
						unk_0x1BC60757AC20ACCF(&(Global_113810.f_14054[Global_20500 /*20*/].f_11), unk_0x4A8C381C258A124D(), 1);
					}
				}
			}
		}
	}

	return;
}

BOOL func_122() // Position - 0x77AE
{
	if (Global_78689)
		if (Global_1836502 || Global_1836503 == 1)
			return true;

	return false;
}

void func_123() // Position - 0x77D5
{
	if (func_82(14))
	{
		if (!unk_0x1C2F771CDC87A3A5(unk_0x4A8C381C258A124D(), 0))
			if (unk_0x4B423FAA24E8ABF0(unk_0x4A8C381C258A124D()) == Global_113810.f_28053[0 /*29*/])
				Global_20500 = 0;
			else if (unk_0x4B423FAA24E8ABF0(unk_0x4A8C381C258A124D()) == Global_113810.f_28053[1 /*29*/])
				Global_20500 = 1;
			else if (unk_0x4B423FAA24E8ABF0(unk_0x4A8C381C258A124D()) == Global_113810.f_28053[2 /*29*/])
				Global_20500 = 2;
			else
				Global_20500 = 0;
	}
	else
	{
		Global_20500 = func_99();
	
		if (Global_20500 == 145)
			Global_20500 = 3;
	
		if (Global_78689)
			Global_20500 = 3;
	
		if (Global_20500 > 3)
			Global_20500 = 3;
	}

	return;
}

